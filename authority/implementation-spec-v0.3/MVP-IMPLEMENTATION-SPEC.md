...
- `contentBriefOutline` 是对已确认 `content_brief` revision 的**结构化提取**，只含三项闭集值：`briefSections`（确定性 `## ` heading 提取；命中 §10.1 段名的归一为英文 canonical 常量，其余经净化；≤12 条、单条 ≤120 字符）、`targetKeywords`（冻结 SearchQuery cluster 的关键词文本，不包含任何 demand 指标；≤50 条、单条 ≤120 字符）、`pageAssignment`（`existing_page|new_asset|mixed|unassigned`）。全部经 Zod `.strict()` 校验；单条净化剔离控制/格式字符与尖括号、折叠空白并截断。brief 的散文正文**永不进入 prompt**。`briefSections` 是 brief 承诺覆盖的主题清单，不是文档结构；文档结构由固定脚手架决定。但它**不是覆盖判定的对象**：命中 §10.1 的段名一律归一为九个英文 canonical 常量，而 validator 又强制要求全部九段，因此对任何合规 brief，`briefSections` 恒等于那九个文档结构标签，拿它去问"正文谈到了吗"是 by construction 恒失败的判据。覆盖判定因此针对 `targetKeywords`（它同样是冻结输入，进 `content_hash`）。提取失败（`briefSections` 为空）必须响亮降级：research pack 记 limitation、coverage claim 记 `failed`、verdict 因此不得为 `passed`、读接口显式呈现。**部分**提取损失（brief 的 distinct `## ` 段数超过 ≤12 上限，或 cluster 关键词数超过 ≤50 上限）同样必须披露而非静默通过，但披露方式与总失败不同，且**不改变 verdict**：research pack 对两种超限各记一条 limitation，说明 brief／cluster 承诺了多少条、实际只有多少条进入 prompt；段数超限时该句子一并追加到 coverage claim 的 detail 里，关键词超限只进 pack limitation。覆盖判定既然针对 `targetKeywords`，一篇覆盖了全部目标关键词的 draft 在段数超限时仍得到 coverage `passed`，verdict 由各条规则各自决定 —— 上限丢弃是被记录下来的事实，本身不是一个降级信号。claim status `failed` 只保留给完全失败与真实的覆盖缺口。（verdict 的取值只有 `passed | needs_review | blocked`，`unevaluated` 是 claim status 而非 verdict。）`english_blog_draft` 使用独立的 `CONTENT_SHADOW_PROMPT_SET_VERSION`，其余三类 prompt 字节不变。
...
- `reviewContentShadowRevision` — 记录一次人工评审：把该 run 产出的 draft revision 标记为已评审（唯一写入 = artifact `draft → ready` 这条既有人工边）。**零外部写入**：本阶段不连接任何 CMS、Git 或第三方发布目标，回执以字段 `externalPublishingWrite: none` 陈述这一点，而不是靠界面文案。评审绑定 revision：`baseRevision` 声明评审者实际读过的版本，交付物已经产生新 revision 时返回 409 `STALE_REVISION` 且零写入，绝不改指向最新正文；自动检查结论若判定的是另一个 revision，同样拒绝。所有守卫在此端重算而非信任调用方：`blocked` 结论不能通过，`needs_review` 结论必须带 `acknowledgeFindings=true`。`blocked` 这条判据由 execution 服务层的**单一模块**持有：通用 artifact 状态 PATCH 的 `draft → ready` 与本端点走同一份实现，不允许两条写入路径各自持有一份会漂移的副本。artifacts 读模型以 `Artifact.adoption` 携带同一个判断（对没有 Content Shadow 质量门的 artifact 类型该字段为 `null`，这与「允许采纳」不是同一句话，读端不得当作后者渲染），使 Studio 的「标记为就绪」控件在被点击**之前**就停用并就地说明原因，而不是让操作者靠被拒来得知。该字段不新增 operation。**没有 decision 参数**：artifact 生命周期的人工边只有 `generating → draft → ready` 与 `draft | ready → archived`，从 `ready` 回退的既有机制就是编辑（追加 revision 并自动退回 `draft`），因此第二个决策值只会是一个「什么都不写却看起来做了决定」的控件。
<!-- API_OPERATIONS_END -->
...
- **AC-001** `pnpm verify:authority` 通过；49 operationId、9 async operation、44 table 与当前应用 ordered migrations 完全一致；0012～0021 累积迁移以精确 bounded executable blocks 纳入 authority schema，最终 migration version 为 `0021_content_shadow_invocation_task`。
