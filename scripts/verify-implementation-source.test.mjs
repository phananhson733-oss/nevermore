...
test("derives active versions and inventories from the reviewed v0.4 lock", () => {
  ...
  assert.equal(lock.migrationHead, "0048_topic_model_generation");
...
test("builds the complete database inventory through the static schema catalog", () => {
  ...
  assert.match(
    verifier,
    /database: 48 migrations through \$\{EXPECTED_MIGRATION_HEAD\}, \$\{EXPECTED_TABLES\.length\} app tables \(pg-boss excluded\), 109 indexes, 156 triggers, and 79 routines in migrate-check/,
  );
