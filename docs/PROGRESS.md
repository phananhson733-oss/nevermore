...
- Migration range: `0001_init.sql` through
  `0048_topic_model_generation.sql` (**48 ordered migrations**), immediately
  after `0047_dataforseo_competitor_metrics.sql`
...
2. Preserve and restore-verify the production backup, then re-check all ordered
   migrations through `0048`; historical proof through `0021` does not prove
