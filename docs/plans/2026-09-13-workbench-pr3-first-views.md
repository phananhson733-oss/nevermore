...
| Q37 | **超限产物不得被静默截断入筐**：`reducer.ts:116-127` 的 `boundArtifact` 以 `truncateUtf16(content, ARTIFACT_CONTENT_MAX)`（20 万 UTF-16 单元）按位置硬切且**不留任何标记**，全仓 UI 无一处提及——JSON 产物会被切成非法 JSON，CSV 切在一行中间，抽屉的复制/下载拿到的就是残缺文本。今天 mock 正文远小于上限不可达，**T10 让用户 GSC 行进关键词矩阵后可达**。裁决：入筐要么存完整 canonical 文本，要么不存并告知，没有第三种；`save()` 超限不派发并由 `ArtifactActions` 渲染提示（成因是量出来的且唯一，可以点名），复制/导出/复制给 AI 照常给全文；`boundArtifact` 截断保留为持久化输入的最后防线。同时 Q23 的 canonical 形态在 `stampArtifact` 规范化（LF 分行、无结尾换行），使 AI 载荷等于 content 由构造保证——`fenceBlock` 共享给约十个 prompt builder，不改它 | codex S2 #1 与「无法确认」段，我核实 `reducer.ts:124` 与全仓无提及 |
| Q36 | **产物的来源声明句必须跟着数据的来源走**（T4 审阅 P3-3）：`workbench.provenance.artifact`（「示例数据：本地生成用于演示，非实测」）由 `useAddArtifact` **无条件**盖到每一个产物上，而 T10 之后 `mock/keywords.ts` 的 `buildRows(seeds, profile, gscRows)` 会把**用户自己导入的** GSC 点击/排名并进关键词矩阵，`keywordMatrixCsv` 再导出。方向虽保守（示例不会被说成实测），但「你导入的真实数据」被标成「非实测」同样是假话，且与 Q6「凡由 GSC 数据派生的标注一律读 `gscRowsSource`/`gscSource`」正面冲突。设计 §6.8 的「固定一行」按此修订。~~**归 T10**：声明句按来源分两版~~。**已落地（分三版）**：`b2c7a015`（i18n：`workbench.provenance.artifact`「示例数据：本地生成的演示结果，不是真实测量；生成于 {at}」、`artifactWithUserGsc`「含你导入的 GSC 数据；其余为本地生成的演示结果，不是真实测量；生成于 {at}」、`artifactWithUnknownGsc`「含来源未知的 GSC 数据；其余为本地生成的演示结果，不是真实测量；生成于 {at}」）、`aa89896b`（主体：草稿必填字段 `gscData: "none" \| "sample" \| "user" \| "unknown"` 由 `artifactGscData(present, source)` 算出，`useAddArtifact` 按它选键，none 与 sample 共用 `artifact`；csv 标记改 `# provenance`，json 键改 `_provenance` 且去掉正文自带的 `_provenance` / `_sampleData`；`usePreparedArtifact` 的 key 含 `gscData`）、`f4fb8ad2`（档案每个标签按自己的正文表态：Markdown 与 AI 上下文取冻结的 `doc.gscSource`，JSON 标签没有 GSC 字段、恒为 none）、`d5909ada`（注释：只有正文含 GSC 派生内容的 builder 读快照 `gscSource`；示例加载器固定用 `artifact`，因为示例站点的 GSC 行就是示例数据）。周报在 `borderline !== null` 时取 `gscRowsSource`，否则 none。codex S36 复核：声明与正文逐件一致。**交接 PR-4**：关键词矩阵 CSV 成为草稿生产方时必须填 `gscData`（必填字段，编译期强制）。旧的已下载产物（`# sample-data` / `_sampleData`）不做兼容，没有读取方。T18 已回写设计 §6.8 | T4 审阅 P3-3 + 我核实 `useAddArtifact.ts:59` 无条件盖章（Q36 之前的行号） |
...
| Q32 修订 | **portal 目标是 `#wb-root`，不是 `document.body`**（T12 执行中发现，我核实采纳）。Q32 与 T12 在这里正面冲突：T12 把 next/font 变量类从 `<html>` 挪到项目 layout 根 `#wb-root` 之后，portal 进 `<body>` 的确认框会**静默**渲染成 fallback 字体。`#wb-root` 是 `#wb-app` 的父节点、从不 inert，所以 Q32 的实质（对话框不落进自己造的 inert 子树）不变；无壳挂载时退回 `document.body`——可达比字体重要。**任何「钉 `parentElement === document.body`」的建议都会把这条修复改回去，不得采纳**（ui 审阅曾如此建议，已作废）。门：有 `#wb-root` 时父节点就是它 / 永远不是 `#wb-app` 后代 / 无 `#wb-root` 时退回 body；真浏览器侧另证遮罩盖满视口（`#wb-root` 或祖先若带 transform/filter/contain 会让 fixed 相对它定位）与对话框内 computed `font-family` 含 Plus Jakarta，归 T12 或 T17 | T12 计划外改动 + ui 审阅 P3 |
...
- [ ] **Step 3b: 示例产物的声明措辞必须与操作者自己的产物同源**（store 审阅 P3-1）— 「盖章只有一处」的清扫护栏（`mock/provenance.fs.test.ts`）守的是 `stampArtifact` 这个**函数名**，不是**措辞**：`mock/demo-artifacts.ts` 的 `DemoDeps.provenanceLine` 由调用方注入，而这个注入点今天零调用者——**本任务就是写下它的那一步**，写成任何别的字符串护栏都不会出声。钉法用跨接缝的行为断言而不是字符串清扫：同一时间戳下，`loadDemo` 产出的示例产物首行声明，必须 `toBe` 由 `useAddArtifact` 为同类型产物盖出的那一行。变异：注入一句手写的声明必须红。
...
- [ ] **Step 1: `week-feed.ts` 测试先行** — `weekFeed(state, now)`：事件来自 `lastAudit` / `lastVis` / `profileDoc` / `kb` / 每件 artifact，过 `weekWindow(now)`（**T8 修复批 `85418522` 改为本地日历日；第三轮 `0796e043` 收成今天与之前 6 天共 7 个本地日期**：副标题首日 00:00 到 now 两端含，副标题 / 产物计数 / 事件流 / 检查卡的范围标记共用这一个边界（检查卡三种说法：范围内不加句；范围外「这次检查不在上面的日期范围内」；时间解析不出「无法确认这次检查是否在上面的日期范围内。」，不写成因），计数与事件流标签称「近 7 天」；事件流另纳入 `auditHistory` / `visHistory`，**按对象引用去重**（第三轮 `b81eabd5`：reducer 只在「A、B、再派发同一个 A」时让 `lastAudit` 与历史共享引用；`kind + at` 会把同一分钟的两次不同运行并成一条。该规则不以「共享引用只在内存里出现」为前提：`loadPersisted` 交入的整份状态不经 `archive`，读回的状态同样可能带共享引用（水合时 `normalizeInterrupted` 让 `audit` 就是 `lastAudit` 那个对象），去重只认对象、不问来路；实测 reducer 在内存里共享的那份报告经 localStorage 读回后是两个独立对象，重载后该重复会列两次，已写进 `week-feed.ts` 与 `week-summary.ts` 注释）），按 `at` 倒序；提及率与事件从 `lastVis.results`（Q20）；KB 缺口用 `kbGapCount`；「较上次（{at}）」带出 `prev.at`。
...
- [ ] **Step 5: jsdom 测试 + 变异** — feed 窗口边界（7 个本地日期：首日 00:00 计入、前一日 23:59 不计入，未来时间被排除；两个时区各跑一次）；全空空态；最终产物文本恰好一处来源声明。**变异方式是往生成结果里逐条插入禁用句（「实测」「已修复」「进前十」等），每插一条未改动的测试必须红**——rev1 写的「删掉断言必须红」在方法上不成立（删断言只会让测试更弱且照样绿）。
- [ ] **Step 6: 提交** — `feat(workbench): 本周变化视图与周报产物`。

## Task 9: 站点档案视图

**依赖：** T1-T4
**Files:** Create `views/profile/{ProfileView.tsx,ProfileInputPane.tsx,ProfileDocTab.tsx,ProfileJsonTab.tsx,ProfileContextTab.tsx,build-profile-doc.ts}` + tests；Modify `app/p/[projectId]/profile/page.tsx`（**执行后实际**：JSON / 上下文两个 tab 合并为 `ProfileTextTab.tsx`；另建 `ProfileOutputPane.tsx`、`profile-artifact.ts`、`useProfileRun.ts` 与测试 harness；修复批把 `usePreparedArtifact` 并入 `hooks/`（周报共用），`setProfileDoc` 携带开跑时的 `basis`，reducer 拒收过期写入）
...
**依赖：** T1-T5
**Files:** Create `views/data-sources/{DataSourcesView.tsx,DataSourcesPanel.tsx,GscImportPane.tsx,GscRowsTable.tsx,real-connections.ts}` + tests；Modify `app/p/[projectId]/data-sources/page.tsx`（**执行后实际**：另建 `GscImportNotice.tsx`、`ClearGscRowsButton.tsx`、`use-gsc-import.ts`、`mock/gsc-import.ts` 与测试 harness；reducer 新增 `clearGscRows { expected: { rows, source } }`，用 `store/same-content.ts` 比较，不匹配时原样返回。T11 与 T18 以此为契约）
...
- [ ] **Step 1b: 导入是否按 query 去重**（codex S7a #3 转交）— `parseGsc` 每条记录一行、不去重；Queries 导出天然每词一行，但拼接或重复导出时同一 query 会有多行，本周页与概览会把它计成多个临界词（`GscRow` 没有页面 / 国家维度可区分）。在导入边界裁决：按 `normQ` 合并（点击 / 曝光求和、均位按曝光加权——见记忆「均位按曝光加权」），或拒收并提示重复；不要让下游各视图各自去重。**执行期裁决（选了第三种）**：按 `normQ` 保留首次出现，不求和、不加权，重复条数单独披露；5000 行上限在去重之后生效；已持久化的旧行不回溯清理。
- [ ] **Step 2: 真实与本地两区** — 上区「真实连接」（只读 + 指向旧页 `sources` 的链接）；下区**「GSC 导入（保存在这个浏览器）」**——rev1 把它叫「示例导入」并挂无条件 `DemoChip`，而这一区唯一的动作是导入用户自己的数据，是无条件的假标注（codex #4）。示例标注只跟着 `gscRowsSource === "sample"` 出现。删「每日 06:00 同步」与 GA4 403 红框（Q15）；GA4 区块明说当前没有模块使用 GA4 数据。**执行期（`a47ec61c`）**：`ClearGscRowsButton` 没有 `onCleared`，hook 也没有 `forget`；结果区在 GSC 行从有变无时由 `use-gsc-import` 清空（无论清空来自本页确认、其他写入还是别的标签页），同时计数加一，文件读取完成时比对计数、变了就丢弃结果。
...
- [ ] **Step 1: 处置两条既有契约测试**（Claude #18）— `SettingsView.test.tsx:70-76` 断言页面含 `inProgressNoLegacy`（本任务要删这句）、`:78-84` 断言页面**没有**任何 `sampleData/sampleSite/sampleTitle`（本任务要给通知块加 `DemoChip`）。**不许整条删**：把 no-chip 断言**收窄到 `[data-wb-real-action]` 子树**（真实操作块内不得出现任何示例标记），并新增「通知块内必须有 DemoChip」。**执行期反转（`77dafb0f`）**：通知块不挂 `DemoChip`——偏好是用户自己的开关，不是示例结果，chip 的 title「示例内容目前仅有中文」与此无关；「通知块内必须有 DemoChip」这条断言随该提交一并反转。
- [ ] **Step 2: `NotifyBlock`** — 四个 `Toggle`（`setNotify` 收整份：`{...state.notify, weekly: next}`）+ 「偏好只保存在这个浏览器，当前不会发送任何通知」（~~+ `DemoChip`~~，见 Step 1 执行期反转）；未 hydrate 时 disabled 或骨架（不渲成「全关」）。
...
- [ ] **Step 1b: 本周变化页**（T8 交接，`dd01a8cc`）— ①周报动作行里 `ArtifactActions` 自带一个 `role="status"`，任何「唯一 `role="status"`」断言必须限定在顶栏子树内，否则在 `/week` 上会误红；②定位属性：`data-wb-week-card`、`data-wb-foot`、`data-wb-summary`、`data-wb-week-feed`、`data-wb-event`、`data-wb-week-borderline`、`data-wb-borderline`、`data-wb-week-next`、`data-wb-step`、`data-wb-week-report`、`data-wb-report-disabled`、`data-wb-week-empty`、`data-wb-skeleton`；③本页有多个 `[data-wb-frame]`（jsdom 已正向断言），e2e 的 Q30 断言要逐个查；④视图内触控目标（事件行 `BUTTON_MINI` 28px、下周先做 `BUTTON_SECONDARY` 36px）归 Step 4b 在 390px 实测。
- [ ] **Step 1c: Tailwind 类名运行时兜底**（T14 交接，`407d291d`..`92554c80`）— T14 的静态护栏（AST + checker 追值流，读不懂就报）有两类已知静态盲区：字面量按位置传给名为 className 的参数、运行时由服务端数据 / i18n 拼出的类。在 e2e 里对每个被访问的工作台页遍历 DOM 的 class token，凡是 Tailwind utility 形态的，断言实际下发的 CSS 里有对应规则（`document.styleSheets` 里能匹配到选择器）；至少覆盖 overview / week / settings / data-sources / profile 与 `/new-project`、`/login`。
- [ ] **Step 1d: shell / views 的真实对比度**（T12 修复批偏离裁决，`13a82dd3`）— 单测对比度门只遍历 `components/workbench/ui/**`：按单个 class 串配对在 shell/ 上失真（深色侧栏底色写在祖先元素上，会被当成白底），在 views/ 上同理，所以 shell / views 今天没有对比度门。改由 e2e 用仓库已有的 `@axe-core/playwright`（见 `e2e/a11y.spec.ts` 的用法）只开 `color-contrast` 规则，在示例已载入的 overview / week / settings / data-sources / profile 与侧栏展开态各跑一次，违规数必须为 0；有意豁免（遮罩等）用选择器排除并在注释写计数。**这是工作台对比度的主门**（codex S8）：`ui/` 的单测对比度门是按 class 字符串静态配对，已知看不见主题覆盖以外的同元素 `cn` / 常量组合、字号（小字复用大字色）、独立 `opacity-*`、带括号的任意 variant、跨元素的文字色与底色——这些只能靠真实渲染抓。所以 axe 检查必须覆盖 `ui/` 原语的真实使用点（确认框打开态、产物操作行的满筐 / 超限提示、StatCard、Toggle），`e2e/a11y.spec.ts` 里对侧栏 color-contrast 的既有排除要先读懂，别照抄成对工作台的整片排除。
...
- [ ] **Step 4d: 壳触控清扫的四个空档**（codex S4 对 `02a0ed03` 的复核，4 条 P2 全部核实成立；今天无实测违规，都是「合理的后续改动会静默漏过」）— 与 Step 4b 同一次改 `e2e/workbench-shell.mock.spec.ts`：
  1. **盒子合格 ≠ 可点**：`sweep()` 只存宽高，`pointer-events-none`、`clip-path`、被邻居盖住都照绿。对「本状态下应可操作」的目标加 `click({ trial: true })`；保留完整矩形并断言同一状态下的目标两两**无正面积相交**。变异：给抽屉 Copy 加 `pointer-events-none` 必须红。
  2. **移动侧栏打开态从没量过**：390 下测试只 `setViewportSize`，从不点汉堡；关闭态侧栏链接是 `inert` + 平移，盒子非 null，量到的是用户碰不到的那一份。打开侧栏、等动画稳定、断言 `inert` 已解除，再对 rail 的精确具名清单做尺寸 + trial click；关闭态另断言不可交互。变异：只在 `mobile && open` 时给 rail 链接 `h-5 py-0` 必须红。
  3. **采样宽度漏掉 `sm`–`md` 段**：640–767 下「+ 新建站点」已显示、汉堡仍在，是两个采样点都看不到的组合。加 639/640 与 767/768 两侧采样，各段写自己的 `ABSENT_AT`；44px 项适用于每个 < 768 的采样点；**抽屉那条测试没有自己的 `setViewportSize`**，参数化到 390 与 1280。变异：「+ 新建站点」改 `py-0 md:py-1` 必须红。
  4. **`:not([hidden])` 是宽预过滤**：它在侧栏和顶栏全局丢掉带 `[hidden]` 的目标，计数却只数顶栏 `a[href][hidden]`，也没核对代理的 `aria-hidden` / `tabIndex=-1` / 真的无盒子。改为先枚举全部 `button, a[href]`，只豁免带专用标记（在 `ProjectSwitcher` 加 `data-wb-nav-proxy`）的精确对象并逐项核对这三个属性与 `boundingBox() === null`；其余 `[hidden]` 目标断言为空清单。变异：给代理加 `class="block"` 覆盖 `hidden` 必须红。
  它明确打不破的（不必再补）：具名清单是 `sort()` + `toEqual` 的双向、保留重数比较；抽屉未打开 / seed 丢失会在「恰好 1 行」「恰好 5 个名字」处红；汉堡去掉 `shrink-0` 得到 `0×44` 时两种引擎返回值都会红。
- [ ] **Step 4e: 软导航后的 CSS 残留**（T12 审阅已实测，结论改写本步）— 从 overview 点顶栏 `a[href='/new-project']` 客户端跳转后，**工作台样式表与 Plus Jakarta 字体仍在页面上**（硬加载时二者都不在）；但 `#main-content` 在 1280/960/900/560/500 下 padding/max-width/min-width/margin 与硬加载逐项相同，标题仍是 Fraunces，与 T12 之前 CSS 全局加载时一致，不是回归。项目页内**没有**客户端跳到 `/login` 的路径（grep 过 `app/p` 与 `components/workbench`），所以不测 `/login`。本步只补一条回归钉子：同一次软导航后，逐属性断言 `/new-project` 关键元素（`#main-content`、主按钮、`h1`）的 computed style 与硬加载相同——`workbench.css` 以后若加了会溢出到非工作台页面的规则，只有软导航这条路能看见。T12 的提交说明与设计稿 §5 里「`/new-project` 不加载工作台 CSS」要限定为「硬加载」。
...
- [ ] **Step 6: 跑法** — `pnpm test:e2e:mock e2e/<file>`（不带 `--`）；`reuseExistingServer: false`。**已修（`5b3dce7e`）**：dist 目录（`.next-e2e-mock-${PORT}`）、blob 目录与 Playwright 输出目录（`test-results/mock-<port>`）都经 `e2e/mock-e2e-lane.ts` 按端口派生，3200 与 3201 并行各跑 new-project-shell 均 3 passed——**现在并行 lane 用 `E2E_MOCK_PORT=3201` 是成立的**。以下为修之前的记录：「并行另跑一套用 `E2E_MOCK_PORT=3201`」当时是错的（T5 与 T13 各自撞了一次，症状是 `ERR_EMPTY_RESPONSE` / `ECONNREFUSED` 与 `.next-e2e-mock` 里 `Cannot find module './vendor-chunks/…'`）：`playwright.mock.config.ts` 把 `NEXT_DIST_DIR` 硬写成 `.next-e2e-mock`，换端口不换 dist 目录，两次并行跑会互相毁掉构建产物。**本任务顺手修掉它**：`NEXT_DIST_DIR` 按端口派生（`.next-e2e-mock-${PORT}`），让并行 lane 真的成立；在此之前只能串行跑，或像 T13 那样在 scratchpad 里放一份自带端口与 dist 目录的配置副本。
...
- [ ] **Step 1b: 「示例数据」chip 的含义**（codex S6 #1 降级转交）— 顶栏与概览页头无条件显示「示例数据」，依据是审计 / 可见度 / 产物无论 `demo` 与否都是本地模拟结果；但 `shell.sampleTitle` 只说「示例内容目前仅有中文」，没说「示例」覆盖的是模块结果而不是用户自己导入的 GSC 行。设计稿 §3.5 与 §6.8 写明 chip 的范围，并裁决它与「来自你导入的数据」脚注同屏时的措辞（改 title 或加限定），两个语种同改。**T11 追加**：设置页通知偏好块初版挂过 `DemoChip`，`77dafb0f` 已移除（偏好不是示例结果）；chip 的 title 是否按挂载点传参，只剩顶栏与概览两处要裁决；`settings.notify.weekly.description` 仍写「本周」，并入 Step 1c ⑤；`inProgressNoLegacy` 键仍被 `PlaceholderView.tsx` 使用，不是死键。
- [ ] **Step 1c: 本周变化页回写与残留**（T8 报告第 5 节）— ①设计稿 §12 的 week 行按实现回写：三卡、摘要行三句（含新增 `week.summaryRow.answerGapsUnknown` / `kbGapsUnknown` 两个「未知」句——ICU `#` 遇非数字渲成 NaN，所以未知只能另起一句）、临界词清单读 `gscRows`（与卡片脚注「最近一次导入」同源，不经 `built` 门控；完全没有 GSC 行时为 `null` 而 `mock/profile.ts` 的 `nearCount` 为 0）、周报 builder 在 mock 层只产正文；②`week.report.title`（「检查结果变化」）在视图里未用（放在动作块上方会说错东西，块标题用 `week.artifactTitle`），裁决：用在正文小节或删键；③**「下周先做」与概览下一步的目标页（audit / answers / keywords / kb / content）在 PR-3 仍是占位页**——占位页带旧版链接所以不是死路，但合入 main 前每个出口都必须有生产者，写进残留表并标归属 PR；④周报的来源声明句无条件「示例数据…不是真实测量」而临界词可能来自用户导入行——Q36，确认 T10 的修法同时覆盖 `module: "week"` 的产物；⑤**第三轮回写**（`0796e043` `2f5b836b`）：范围是 7 个本地日期，计数与事件流称「近 7 天」，检查卡在范围外时只说「这次检查不在上面的日期范围内」；页面标题 / 导航「本周变化」、空态「这周还没有结果」「这周没有可写进周报的结果」、字段名 `artifactsThisWeek` 在此统一裁决命名；设计稿 :294「7 天窗口用 `withinDays`」改为 `weekWindow`；⑥检查卡未消费 `incomparableAt`（只省略增减，不说为何不比，codex S7r2 P3）——补一句说明或记残留；⑦事件流按对象引用去重，引用经 JSON 往返失效，重载后重复条目会列两次，写进 §12 已知限制；⑧产物筐「清空」无确认框，沿用原型（S6r3 修后只删点击时画面上的 id），交 Owner 裁决是否加确认。
- [ ] **Step 1d: Tailwind 扫描范围写进设计稿**（T14 报告「计划写错」三条）— `workbench.css` 现在有两个 `@source`（`components/workbench` + `app-shell.module.css` 的主题变量读取所需根），`@theme inline` 下变量只有被扫描到才进 `:root`；类值藏在 `.ts` 常量、CSS `var()` / `composes`、`packages/*` 里，护栏按值流读取而不是按 tsx 文件清单；`workbench-css.test.ts` 的两处 `@import` 字面量随之改过。写明两类静态盲区与 T17 Step 1c 的运行时兜底。
...
**T18 执行期注记（2026-09-14，代码基准 `d5909ada`）：**

- **计划文档自此由 T18 负责**，main 不再同时编辑。设计稿升 rev8；每一句回写都先对照代码核实，交接文件只作线索。
- **已回写**：设计稿 §4.1（原语、hooks、视图与 lib 新文件）、§4.3（站点卡真实状态、清除示例按钮、chip 的范围与挂载点）、§5（项目 layout 挂 CSS 与字体、`@theme inline`、Tailwind 扫描范围与静态盲区、「不加载工作台 CSS」限定为硬加载）、§6.1（`gscRowsSource`、`ProfileDoc.gscSource`、`conns` 死字段）、§6.4（reducer 的 PR-3 修订）、§6.5（第四、第五项豁免，最后写入者赢对用户的代价，夏令时歧义）、§6.7（按值判定、确认范围全集、动态 import、确认绑定内容）、§6.8（三版来源声明、盖章形状、规范形态、入筐不截断、Markdown 导出合同）、§7；PR-2 计划残留表（Step 2）；`docs/PROGRESS.md`（Step 3，验证数字留给 T19）。
...
- **待 Owner 裁决**：【6】产物筐「清空」与单件「删除」无确认也不能撤销（残留表已加行，PR-3 描述列为待定项）。
- **T18 发现的计划与代码不一致**（已写进残留表或设计稿）：
  - Q35 要求的「`ui/` 不得 import `shell/`」导入图钉子没有落地；
  - Q6 ④ 的侧栏徽标注释没有落地；
  - Step 1b 所说「chip title 只剩顶栏与概览两处要裁决」不准确，DemoChip 还挂在本周、档案页头、档案文档小节与占位页，只是与「你导入的数据」脚注同屏的是顶栏与概览；
  - 计划 Step 1b 引用的设计稿「§3.5」不存在，chip 范围写在 §4.3 与 §7；
  - 计划多处写 `store/types.ts`，实际文件是 `lib/workbench/types.ts`。
- **随后回写**：设计稿 §6.3（`formatShare` 共用、概览读 `lastVis` 与侧栏徽标读 `visResults` 暂时不同）、§10（Q19 / Q25 / Q15 / Q16 / Q5 / Q24）、§12（五视图与产物中心）、§14（PR-3 评审处置：计划评审 45 条与各跨模型攻击面的发现数，逐条处置仍只在 scratchpad，PR 描述要带上）。
- **T18 未做、不属于文档的项**：§6.5 披露的最后写入者赢代价没有进界面与存入回执（归 PR-4）；§8 的 PR-3 模块流与生产冒烟由 T17 / T19 执行，本稿不写结果。
...
| **顶栏左侧在 390px 下装不下**（T13 实测）：汉堡按钮改用 `h-11 w-11` 后量出 **0x44**——`.wb-reset` 的 `svg { max-width: 100% }` 让按钮的 min-content 宽度为 0，flex 把唯一能压的那一项压到了零。`shrink-0` 修掉了症状，**溢出本身还在**，占地的是项目切换器。注意 `mobile-shell` 那条「390px 无横向溢出」正是**靠把一个控件压成零宽**才满足的——门通过了，理由是错的；现在 T13 的命中区清扫（宽高都 ≥24）会抓住同类复发 | **已在 PR-3 关闭**。T17 实测发现这是生产缺陷，不只是溢出：示例态 390px 下 chip 盖住汉堡，移动端打不开导航；768–1024px 横向溢出 17px。修复为 `<1280px` 两行布局（方案 Z，`4af1ef6c`），存储失败态修复与 `<768` 切换器 44px 见 `bb8eadbc`，1px 探出与焦点环见 `b8915323`。几何、Tab 顺序、焦点环与存储失败态由 flow / shell spec 钉住，变异全红。布局说明写在设计稿 §4.3「顶栏两行布局」 |
...
| **跨标签并发写入整份覆盖**（codex S2r3 #1）：两个标签页各存一件产物，后写者的快照经 `storage` 事件整份替换先写者，先存的那件消失而界面仍「已存入」；同理任何两标签并发编辑都会丢一边。设计稿 §6.5「最后写入者赢，不合并」的已知代价，但设计稿没有对用户披露，也没写到产物成功回执 | T18 已写进设计稿 §6.5 披露（2026-09-14）；**界面与存入回执仍未披露**；写盘边界串行化 / 回执来自持久化边界归 PR-4（与运行租约同批） |
| **临界词区间的文字定义**（codex S7a #8）：`gscStatus` 判 `10 < position <= 30`，文案全模块写「11-30 名」，均位 10.5 会出现在「11-30 名」清单里 | ~~T18 统一措辞~~ **已裁决**（main 2026-09-14，T18 方案【4】候选 A）：判据不改，所有「11-30」改区间写法——标签「临界词（排名 >10 且 ≤30）」、图例「已排名 ≤10 · 临界 >10 且 ≤30 · 缺口 >30」、句子「推进 {count} 条临界词（排名 >10 且 ≤30）」（en 同形），mock 中文正文 `profile.ts` / `week.ts` 同步。**已实现**：`d6d3764c`（导出文档）+ `23c0fe53`（i18n 七处，`views-i18n.test.ts` 用 FORBIDDEN_BY_KEY 禁旧区间拼写）；`fae6e640` 再改概览测试整句与 `views/week/week-summary.ts` 的注释；T18 在该提交上核实：非测试代码与消息目录里「11-30」零命中。关键词视图（PR-4）沿用同一写法 |
| **本地分钟时间串遇夏令时回拨有歧义**（codex S7a #10）：`at` 为无时区 `YYYY-MM-DD HH:mm`（schema 正则钉死），回拨那一小时的事件解析成早一小时，7 天窗口会漏计、同小时内排序会颠倒。检查卡的范围说明在跳时缺口和回拨重复小时里说「无法确认」（`b7e8b8ad`，按两种读法判定），计数过滤不变 | T18 已写入设计稿 §6.5（已知限制）；改存储格式要升 `PERSISTED_VERSION`，归下一次持久化形状变更 |
| **CSV 引号单元格内的 CR 会被规范成 LF**（T3 偏离 1 的副作用）：为了让「AI 载荷与原文逐字相同」由构造保证，`stampArtifact` 把整份输出规范成 LF、无结尾换行，引号内的 CR 一并改写。设计稿 §6.8 要把这写成通则，而不是只说「`toCsv` 不以换行结尾」 | T18 已写入设计稿 §6.8（`71902e31`，`mock/provenance.ts` 的 `canonicalText`） |
...
| **`views/data-sources/real-connections.ts` 反向依赖 `shell/gsc-connection.ts`**（T10）：视图层依赖了壳层文件，应移到 `lib/workbench` 并加 provider 参数 | T16 没有移动（代码基准 `d5909ada` 上 `real-connections.ts` 仍 `import { gscConnectionState } from "../../shell/gsc-connection.ts"`）→ **PR-4**：接真实数据时移到 `lib/workbench` |
| **档案生成途中被其他标签页整份替换状态**（T9）：`basis` 守卫只挡本标签的替换，跨标签 LWW 下仍可能后写覆盖 | PR-4（运行租约，与跨标签归属同批） |
| **导出 markdown 里，代码 span 和尖括号链接目标会显示 `&lt;`**（codex S7r3 裁决）：`docText` 对 `<[A-Za-z!?/]` 一律编码为实体。原来的放行扫描器连续两轮被 GFM 裸链接绕过，已撤销；用户自己写的实体仍按实体渲染。**导出合同**（codex S9r2b 裁决）：用户与 AI 的值保留行内 Markdown 语义（链接、强调、代码 span、反斜杠转义、实体照 Markdown 渲染；gfm 下 `<https://…>` 的链接目标会带上末尾 `%3E`），只中和 HTML 与块结构（标题、列表、引用、代码块、链接定义）；单行字段折叠空白。不做逐字符实体编码：llms.txt 和复制给 AI 的正文要保持可读。标题行末尾 `#` 串（`1454a151`）与档案头部说明行（`faf452b4`）同批，codex S9r3 复核 0 findings | T18 已写入设计稿 §6.8（已知限制与合同全文；代码侧合同在 `mock/builders/compose.ts`，`d638ed12`） |
| **`visComplete` 重复派发不去重**（codex S6r4）：reducer 对同一次可见度结果重复派发不做幂等处理；今天没有生产派发者，不可达 | PR-4（接真实可见度运行时，与运行租约同批） |
| **别的标签页清空 GSC 行，会顺带丢掉本页正在读的文件**（T10 #12）：`use-gsc-import` 以「行从有到无」计数作废进行中的读取，跨标签清空同样计数。裁决与确认清空一致（保守，不回填别处删掉的行）；只有间接测试覆盖。反方向（codex S10r2 F1）：计数比较的是两次渲染之间的有无，同一批更新里合并掉的中间清空不计数（同标签页的清空都在 `flushSync` 里，只有跨标签连续 storage 事件能造出），此时文件照常写入，与「替换现在保存的行」一致；reducer 层清空版本号归 PR-4 | T18：设计稿 §6.5 披露（已写入） |
| **产物筐「清空」与单件「删除」没有确认也不能撤销**，与清除示例、清空 GSC 行、载入示例覆盖不一致（`shell/ArtifactDrawer.tsx` 按点击时画面上的 id 派发 `clearArtifacts`，`540ddd35`；单件删除派发 `removeArtifact`）。候选 A：加 `ConfirmDialog`，新增三个键 `shell.drawer.clearConfirm.{title,body,ok}`，正文里的件数取点击时的 ids（与派发的 ids 同一份）；风险是确认框叠在抽屉 Dialog 之上——`ui/Dialog.tsx` 按计数给 `#wb-app` 加 inert、支持严格嵌套，但抽屉本身在 `#wb-app` 之外、确认框 portal 到 `#wb-root`，两层的焦点圈与 Esc 归属没有测试，e2e 里「Dialog 根恰好 1」的断言在打开态会变成 2。候选 B：不加确认，改为可撤销的回执（「已清空 N 件 · 撤销」），需要新的 reducer action，范围更大 | 待 Owner 裁决（PR-3 描述里列为待定项）；设计稿按当前行为（无确认）写并注明待裁决。**若选候选 A，先修**：`ui/Dialog.tsx` 在下层 Dialog 上再叠确认框时，关闭上层不向下层回焦（交付前 a11y 验收 #1；今天没有任何 Dialog 上会叠确认框，所以 PR-3 未修） |
| **档案 Markdown 不含「一句话定位 / 核心功能 / 竞品」**：标题行的品牌、站点、市场取当前项目，正文是生成时的快照（头部固定说明行，`faf452b4`）；`positioning / features / competitors` 不在 `ProfileDoc` 快照里，写进正文就会说成快照时的值而实际是当前值，冻结它们需要改持久化形状 | 下一次持久化形状变更（要升 `PERSISTED_VERSION`） |
| **概览「接下来做什么」与本周「下周先做」的出口在 PR-3 仍是占位页**：目标段名 `audit / visibility / answers / keywords / kb / content`（`views/overview/next-steps.ts`、`views/week/week-summary.ts`）全部渲染 `PlaceholderView`。占位页带旧版链接（`kb / visibility` 除外，`LEGACY_LINKS` 为空，说「页面开发中」），不是死路，但合入 main 前每个出口都必须有生产者 | `audit / visibility / keywords` → PR-4；`answers / kb / content` → PR-5；PR-3b 合 main 前复核 |
| **本周检查卡不说明为什么不给增减**（codex S7r2 P3）：`week-summary.ts` 在上一次检查与最近一次的页面集合不可比时算出 `incomparableAt`，周报正文据此写「上一次检查（{at}）查过的页面不能确认和最近一次（{at}）相同，这里不做对比。」（`mock/builders/week.ts`），视图的检查卡不读它，只是省略增减 | 需要补键；随下一次本周页文案批次，或 PR-4 |
| **本周事件流按对象引用去重，读回后重复条目会列两次**：reducer 在内存里让 `lastAudit` 与历史共享引用时（「A、B、再派发同一个 A」），经 localStorage 读回是两个独立对象，重载后同一次运行列两次；按 `kind + at` 去重会把同一分钟的两次不同运行并成一条，更糟（`b81eabd5`、`dffac550`）。同源的另一个形状：若交进来的状态中 `auditHistory` 最后一条恰好就是 `lastAudit` 对象，健康分卡会与自己比较显示 +0（`week-summary.ts` 的 `health()` 直接取 `auditHistory.at(-1)`）；reducer 的 `archive` 不产出这种形状，实测从 localStorage 读回也达不到 | 已知限制（设计稿 §12 本周行）；真正的修法是运行 id，要改持久化形状 |
| **Q35 的导入图钉子没有落地**：计划要求在 T16 的导入图护栏里钉住「`ui/` 不得 import `shell/`」。代码上这条边已消失（`0409f7d9` 后 `ui/` 下无 `shell/` 导入），但 `store/client-import-graph.test.ts` 没有对应断言，重新加回这条边不会红 | **已补**（`37ae84c2`）：`client-import-graph.test.ts` 新增直接边规则，`ui/` 下 22 个模块、41 条边，落进 `shell/` 的 0 条；fixture 控制用例必须恰好报出两条（静态 `@/`、动态相对路径）；变异「`ui/Chip.tsx` 加静态相对导入」「`ui/Delta.tsx` 加动态 `@/` 无扩展名导入」都红。经第三个目录间接到达 `shell/` 不在规则内，头注释写明 |
| **Q6 ④ 的注释没有落地**：计划要求在侧栏 `dataSources` 徽标处写明「纯数字，不标示例」。徽标值是 `selectors.ts` 的 `countOrNull(state.gscRows.length)`，`shell/Sidebar.tsx`、`shell/workbench-nav.ts`、`store/selectors.ts` 里都没有这句注释；行为本身符合裁决（纯数字） | **已补**（`acb0e243`）：注释写在 `shell/workbench-nav.ts` 的 `dataSources` 条目上方（`Sidebar.tsx` 对所有徽标共用一段渲染代码，不适合写单个徽标的说明） |
| **Dialog 不锁背景滚动**（交付前 a11y 验收「无法确认」）：`ui/Dialog.tsx` 只对 `#wb-app` 设 `inert`，不锁 document 滚动。移动端可能滚动穿透，关闭后也不恢复滚动位置；需要真实设备确认 | 随下一批带长内容对话框的视图（PR-4 / PR-5），在真机上确认后决定 |
| **ConfirmDialog 正文没有 `aria-describedby`**（交付前 a11y 验收）：焦点落在取消按钮上时，读屏不保证自动读出后果说明，正文要浏览才能读到 | PR-4（与产物筐清空确认同批，如果 Owner 选 A） |
| **竖屏平板在存储失败时只看到图标**（768–1023px，`bb8eadbc` 方案 A 的已知取舍）：整句由 `role="status"` 播报，视觉上只有 16px 琥珀色图标加悬停提示，触屏无法悬停 | 已接受。若要改，给图标加可点开的说明，归布局批次 |
