...
| **Q35 的导入图钉子没有落地**：计划要求在 T16 的导入图护栏里钉住「`ui/` 不得 import `shell/`」。代码上这条边已消失（`0409f7d9` 后 `ui/` 下无 `shell/` 导入），但 `store/client-import-graph.test.ts` 没有对应断言，重新加回这条边不会红 | 待 main 指派（T18 发现；PR-3 交付前补一条测试，或记入 PR 描述） |
| **Q6 ④ 的注释没有落地**：计划要求在侧栏 `dataSources` 徽标处写明「纯数字，不标示例」。徽标值是 `selectors.ts` 的 `countOrNull(state.gscRows.length)`，`shell/Sidebar.tsx`、`shell/workbench-nav.ts`、`store/selectors.ts` 里都没有这句注释；行为本身符合裁决（纯数字） | 待 main 指派（T18 发现，只差注释） |
