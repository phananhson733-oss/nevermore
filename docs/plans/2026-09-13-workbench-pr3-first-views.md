...
- [ ] **Step 1c: 本周变化页回写与残留**（T8 报告第 5 节）— ①设计稿 §12 的 week 行按实现回写：三卡、摘要行三句（含新增 `week.summaryRow.answerGapsUnknown` / `kbGapsUnknown` 两个「未知」句——ICU `#` 遇非数字渲成 NaN，所以未知只能另起一句）、临界词清单读 `gscRows`（与卡片脚注「最近一次导入」同源，不经 `built` 门控；完全没有 GSC 行时为 `null` 而 `mock/profile.ts` 的 `nearCount` 为 0）、周报 builder 在 mock 层只产正文；②`week.report.title`（「检查结果变化」）在视图里未用（放在动作块上方会说错东西，块标题用 `week.artifactTitle`），裁决：用在正文小节或删键；③**「下周先做」与概览下一步的目标页（audit / answers / keywords / kb / content）在 PR-3 仍是占位页**——占位页带旧版链接所以不是死路，但合入 main 前每个出口都必须有生产者，写进残留表并标归属 PR；④周报的来源声明句无条件「示例数据…不是真实测量」而临界词可能来自用户导入行——Q36，确认 T10 的修法同时覆盖 `module: "week"` 的产物。
