...
| **本地分钟时间串遇夏令时回拨有歧义**（codex S7a #10）：`at` 为无时区 `YYYY-MM-DD HH:mm`（schema 正则钉死），回拨那一小时的事件解析成早一小时，7 天窗口会漏计、同小时内排序会颠倒。检查卡的范围说明在跳时缺口和回拨重复小时里说「无法确认」（`b7e8b8ad`，按两种读法判定），计数过滤不变 | T18 设计稿披露为已知限制；改存储格式要升 `PERSISTED_VERSION`，归下一次持久化形状变更 |
...
| **`views/data-sources/real-connections.ts` 反向依赖 `shell/gsc-connection.ts`**（T10）：视图层依赖了壳层文件，应移到 `lib/workbench` 并加 provider 参数 | T16（导入图）或 T18 裁决；不移就留在残留表 |
| **档案生成途中被其他标签页整份替换状态**（T9）：`basis` 守卫只挡本标签的替换，跨标签 LWW 下仍可能后写覆盖 | PR-4（运行租约，与跨标签归属同批） |
| **导出 markdown 里，代码 span 和尖括号链接目标会显示 `&lt;`**（codex S7r3 裁决）：`docText` 对 `<[A-Za-z!?/]` 一律编码为实体。原来的放行扫描器连续两轮被 GFM 裸链接绕过，已撤销；用户自己写的实体仍按实体渲染。**导出合同**（codex S9r2b 裁决）：用户与 AI 的值保留行内 Markdown 语义（链接、强调、代码 span、反斜杠转义、实体照 Markdown 渲染；gfm 下 `<https://…>` 的链接目标会带上末尾 `%3E`），只中和 HTML 与块结构（标题、列表、引用、代码块、链接定义）；单行字段折叠空白。不做逐字符实体编码：llms.txt 和复制给 AI 的正文要保持可读 | T18：设计稿 §6.8 披露为已知限制 |
| **`visComplete` 重复派发不去重**（codex S6r4）：reducer 对同一次可见度结果重复派发不做幂等处理；今天没有生产派发者，不可达 | PR-4（接真实可见度运行时，与运行租约同批） |
| **别的标签页清空 GSC 行，会顺带丢掉本页正在读的文件**（T10 #12）：`use-gsc-import` 以「行从有到无」计数作废进行中的读取，跨标签清空同样计数。裁决与确认清空一致（保守，不回填别处删掉的行）；只有间接测试覆盖。反方向（codex S10r2 F1）：计数比较的是两次渲染之间的有无，同一批更新里合并掉的中间清空不计数（同标签页的清空都在 `flushSync` 里，只有跨标签连续 storage 事件能造出），此时文件照常写入，与「替换现在保存的行」一致；reducer 层清空版本号归 PR-4 | T18：设计稿 §6.5 披露 |
