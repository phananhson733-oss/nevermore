...
- [ ] **Step 1: `week-feed.ts` 测试先行** — `weekFeed(state, now)`：事件来自 `lastAudit` / `lastVis` / `profileDoc` / `kb` / 每件 artifact，过 `weekWindow(now)`（**T8 修复批 `85418522` 改为本地日历日；第三轮 `0796e043` 收成今天与之前 6 天共 7 个本地日期**：副标题首日 00:00 到 now 两端含，副标题 / 产物计数 / 事件流 / 检查卡的范围标记共用这一个边界，计数与事件流标签称「近 7 天」；事件流另纳入 `auditHistory` / `visHistory`，**按对象引用去重**（第三轮 `b81eabd5`：reducer 只在「A、B、再派发同一个 A」时让 `lastAudit` 与历史共享引用；`kind + at` 会把同一分钟的两次不同运行并成一条。引用经 JSON 往返即失效，重载后该重复会列两次，已写进注释）），按 `at` 倒序；提及率与事件从 `lastVis.results`（Q20）；KB 缺口用 `kbGapCount`；「较上次（{at}）」带出 `prev.at`。
