...
- [ ] **Step 4e: 软导航后的 CSS 残留**（T12 未验证项；若 T12 审阅已实测结清则勾掉并注明出处）— 从 `/p/<id>/overview` 用站内链接软导航到 `/login`（非硬加载），断言 `document.styleSheets` 里不再有工作台选择器、页面上没有 `.wb-reset` 规则生效；若 App Router 保留了样式表，逐属性比对 `/login` 关键元素在硬加载与软导航两种到达方式下的 computed style 相同。
