...
- [ ] **Step 1b: 「示例数据」chip 的含义**（codex S6 #1 降级转交）— 顶栏与概览页头无条件显示「示例数据」，依据是审计 / 可见度 / 产物无论 `demo` 与否都是本地模拟结果；但 `shell.sampleTitle` 只说「示例内容目前仅有中文」，没说「示例」覆盖的是模块结果而不是用户自己导入的 GSC 行。设计稿 §3.5 与 §6.8 写明 chip 的范围，并裁决它与「来自你导入的数据」脚注同屏时的措辞（改 title 或加限定），两个语种同改。**T11 追加**：设置页通知偏好块也挂了 `DemoChip`，它的 title「示例内容目前仅有中文」和通知偏好无关，同批裁决 chip 的 title 是否按挂载点传参；`settings.notify.weekly.description` 仍写「本周」，并入 Step 1c ⑤；`inProgressNoLegacy` 键仍被 `PlaceholderView.tsx` 使用，不是死键。
