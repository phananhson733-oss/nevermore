...
- Final same-evidence comparison favored low reasoning over none: a 19.226-second low call retained separate definition/input/procedure needs, the relevant PAA, direct question-specific excerpts and a tentative inferred-profile angle. None was faster but repeatedly conflated needs or broadened citations. The exact Luna caller selects low; shared transport defaults, temperature, 30-second deadline, 4000-token cap and one-call policy are unchanged. Rationale/why drafting targets 240 code points to leave margin below the unchanged strict 400 cap.
