...
六个修正（1–3 来自架构评审 §7.6 / §3.2 / §1.2；4–6 来自第二轮 codex 评审 2026-08-03，见附录 C）：
...
4. **窗口按 Pacific Time 切，不按服务器 UTC**。Search Analytics 的自然日是 PT。用 UTC 直接切 28 天窗，边界日会漂移一天，`dataState: final` 也会跟着错位。查询计划里每个窗的 `startDate`/`endDate` 必须由 PT 日历日推导，且该推导要有独立单测（跨 DST 切换的两个日期各一条）。
5. **`aggregationType` 必须校验实际返回值，不能假设**。一旦按 `page` 分组或过滤，Search Analytics 不接受 `byProperty`，服务端会改用自己的聚合口径并在响应里回 `responseAggregationType`。`pageTotals`（`[page]`）与 `ctrCurve`（`[query]`）因此是**两种不同聚合口径**，它们的总量和份额不可互相相除。reader 必须读回 `responseAggregationType` 并随数据带出；任何跨口径的分母计算是缺陷，不是近似。
6. **`[page,query]` 不是 `[query]` 的可加展开**。Google 在按 page 和/或 query 分组时会为控制计算量丢行，只保证返回受内部限制约束的 top rows，且排序按 clicks 降序（不是曝光）。`pageQueries` 的行集因此既不闭合也不是曝光完备的，**分页到底也补不回来**。凡是依赖"这些行加起来就是全部"的判断（份额、主导页、覆盖占比）都必须先算覆盖率 = `pageQueries` 曝光 / 对应 `[query]` 或 `[page]` 曝光，覆盖率不足则返回 `insufficient_evidence`（见 §二.4 新增码）。
...
| 站内措辞模式对照（Owner 2026-08-03 新增，见 §2.5） | "你站内在相近位置拿到更高 CTR 的页面这样措辞；照同样模式，这页可以写成 X" | "把标题改成 X 就能提高 CTR" |
| （未来）有 SERP 抽样特性 | "当前抽样 SERP 出现 X 特性，可能压低点击" | "X 是过去 28 天低 CTR 的确定原因" |
| （未来）有改版前后对照 | "建议测试 title/meta 变体" | "改完一定提升" |

v1 停在**第二行**（Owner 2026-08-03 决定扩大范围；原 v3.1 停在第一行）。"同页其他查询健康"不再作为放行"改标题"的闸门（codex §2.1：同页不同查询的 SERP/意图完全可以不同，不构成反事实），仅作为证据表的展示列。

第二行与第一行的关键区别：**草稿的证据基础是站内措辞模式，不是低 CTR 的成因**。工具不知道这页 CTR 为什么低，也不声称知道；它只知道同一个站里哪些页在相近位置拿到了更高 CTR、它们的标题怎么写。这个对照可以从第一方数据得出，而"低 CTR 是因为标题"不能。
...
第二轮 codex 评审新增两码（2026-08-03，见附录 C）：

- `low_dimension_coverage` —— `[page,query]` 返回曝光 / 对应 `[query]` 或 `[page]` 曝光的覆盖率低于阈值。触发时该页面的逐查询明细不出结论。存在的理由：Google 在细维度分组时丢行，一份不完整的行集看起来和一份完整的行集完全一样，不显式算覆盖率就会把"没返回"读成"不存在"（§一修正 6）。
- `brand_segment_insufficient_evidence` —— 品牌查询组样本不足最小桶质量。触发时**不建品牌曲线、不做品牌/非品牌比较、不输出品牌相关发现**，而不是拿两三条品牌查询画一条曲线。实测站点品牌查询仅 2 条，这是 ICP 常态而非异常。

### 2.5 Title/Meta 草稿（Owner 2026-08-03 决定纳入 v1）

**这是对 §零"砍掉的"清单的显式撤销**，理由是落地页文案（`2026-07-30-落地页文案-p0-1-seo-quick-wins.md`）把草稿作为核心钩子，Owner 选择扩大 M2 范围兑现它而非改文案。撤销的只有草稿本身，§零 其余砍单（自动因果归因、AIO 折算、全事件报告、跨工具自动闭环、跨请求缓存）继续有效。

**证据基础（决定了它能说什么）**：草稿由三份第一方材料生成 —— 目标页当前 title/meta、同站在相近位置桶拿到更高 CTR 的对照页的 title/meta、以及该页在 GSC 里的实际查询词。它回答的是"同一个站里赢得点击的页面怎么措辞"，不是"这页为什么没赢"。

**新增能力与成本**：GSC 不提供 title/meta，必须爬取。爬取范围硬性封顶为**证据表前 N 条候选页 + 其对照页**（N 冻结在 Phase 0-A，建议 5），不做全站爬取。LLM 调用一次批量生成，不逐页调用。

**必须成立的边界**：

1. 草稿是**候选措辞**，不是建议。UI 与文案都不得出现"改成这个就能提升"或任何效果承诺。
2. 对照页必须**具名可见**——用户要能看到草稿是照着自己站的哪个页面写的。一份说不出出处的草稿等于一份通用模板，而通用模板正是 §零 砍掉它的原因。
3. 找不到合格对照页（同位置桶内没有 CTR 显著更高且样本足够的页）时，**该条不出草稿**，报 `no_comparable_high_ctr_page`，不退化成通用模板。
4. 不落库，因此**没有改版前后对照**。分级表第三行仍然关闭：不得出现"测试变体""看看是否提升"这类暗示工具会帮你测量的表述。落地页 FAQ 里"改完留在 project 看效果"这句因此必须删除或改写（公开工具零落库）。
5. 草稿不改变证据表的排序、不产生新的"命中"判定。它挂在已有的证据表行上，是行的一个可选附件。
...
| P0-1 | 单端点 | ~30s 证据表（baseline 3–8 + [page,query] + 计算）**+ 20–40s 草稿**（爬 ≤2N 页 + 一次批量 LLM）= **50–70s** | 120 |
...
- **GSC 单飞键跨工具共用（第二轮 codex 评审新增，2026-08-03）**：P0-3 现在用 `tools:traffic-drop:inflight:<ip>`。若 P0-1 再起一个自己的键，同一个 IP 就能同时占住两条 Search Console 连接。**Search Console 配额按 GCP project 计，不按访客**，所以这不是访客自己变慢的问题——是一个访客花掉其他所有访客的额度。所有读 GSC 的公开工具必须共用一个键 `tools:gsc:inflight:<ip>`，P0-3 的现有键一并迁移。
- **单飞是 per-isolate best-effort，不是严格 per-IP**：`public-tool-request.ts:81-102` 的 `activeSlots` 是模块级 `Set`，源码注释已声明这是有意为之（严格闸门需要共享存储，与零落库架构冲突）。这条**不是缺陷**，但对外文案不得把它描述成硬限额，威胁模型也不能假设它拦得住并发滥用。
...
| 1 授权层 | 8–12 | 8–12 | 未触及 |
| 2 P0-1 | 5–8 | 7–11 → **11–16** | baseline + 站点级压制接线 + 反例 fixture；**+4–5 为 Title/Meta 草稿**（页面爬取复用 P0-2 crawler 1–2、对照页选取 1、LLM envelope + 边界测试 2） |
...
- [x] ~~P0-1 v1 不含 Title/Meta 草稿、不含"改标题"建议（证据表产品）—— 落地页文案需按此写~~ —— **已决且反转（Owner 2026-08-03）：v1 纳入 Title/Meta 草稿**，见 §二.5。落地页文案的草稿钩子保留，但草稿定位为"站内措辞模式对照"而非诊断结论；分级表第三行（前后对照）仍关闭，AIO 折算仍不做。Phase 2 从 7–11 升到 11–16
...
## 附录 C · 第二轮 codex 评审裁决表（2026-08-03，M2 开工前）

**背景**：Owner 在 M2 开工前重新讨论了 P0-1 的产品形态，提出四项在冻结基线之上的新增/修改。全部提交第二轮 codex 评审（codex session `019fc669`，read-only，含 Google 官方 API 文档核查）。

**四项提案全部被否**，理由不是"阈值没调好"，而是证据、样本量、GSC 数据完整性三条同时不成立。

| # | 提案 | 裁决 | 主要理由 |
|---|---|---|---|
| C1 | 新增检测器 D2「正在上升但无人管」：4 个 28 天块的位置单调改善 + 曝光增长，行动标签"只盯着别动" | **拒绝** | 块平均位置不能解释为排名运动（与 §一修正 1 同源问题：GSC 只给曝光加权平均位置，位置改善可能纯由展示构成变化产生）。ICP 小站预期检出接近 0。"只盯着别动"仍是行动建议——若真实原因是需求增长或 Google 改由另一 URL 承接，不动就是错的 |
| C2 | 新增检测器 D4「页面-查询错配」：同一 query ≥2 page 有曝光且无绝对主导 ⇒"需合并页面" | **拒绝** | `[page,query]` 会丢行且只返回 top rows，"未观察到主导页"可能只是主导页那行没返回（§一修正 6）。且多 URL 承接同一 query 有至少四种无害解释（国家/设备差异、canonical 归并、不同搜索呈现、合理内容层级）。"需合并页面"比冻结方案已禁止的"改标题"更越界 |
| C3 | CTR 基准分品牌/非品牌两条曲线 | **拒绝**，降级为排除 | 实测站点品牌查询仅 2 条，建不出满足"桶质量 ≥500"+ 留一法的曲线，画出来是伪造精度。且品牌词表来源未冻结（拼写/产品名/创始人名/歧义词/多语言），错分会同时污染两条曲线。改为：品牌查询从非品牌证据表中排除；样本不足报 `brand_segment_insufficient_evidence`（§二.4） |
| C4 | 历史数据用 4×28 天块而非 12 周块，理由是"28 天块样本量是周块 4 倍" | **拒绝**，论证不成立 | 三处错：(a) 样本量只说明每个点更大，D2 判断的是点与点之间的变化；(b) 比较不公平，4×28=112 天 vs 12×7=84 天，多看了 28 天历史；(c) 28 天平滑恰好抹掉"最近才开始上升"这个 D2 自己声称要抓的信号 |

**净结论**：P0-1 v1 保持 §二 冻结形态——层 1 连续披露的 CTR 证据表，不做二值命中判定，不出行动建议。C1/C2 若未来重提，前置是 20+ 站真实 GSC spike，带可审计标签（D2：后续 28/56 天是否继续改善；D4：人工 canonical/意图审查是否真支持合并），报告空集率、误报率、API 行数/分页/时长/429 率。

**本轮真正的产出**是四条冻结基线自身也漏掉的硬约束，已回填正文：

| 落点 | 约束 | 不修的后果 |
|---|---|---|
| §一修正 4 | 窗口按 Pacific Time 切，不按服务器 UTC | 边界日漂移一天，`dataState: final` 跟着错位 |
| §一修正 5 | 按 page 分组/过滤后不接受 `byProperty`，必须读回 `responseAggregationType` | `pageTotals` 与 `ctrCurve` 是两种聚合口径，相除得到的份额是错的 |
| §一修正 6 | `[page,query]` 不是 `[query]` 的可加展开，必须算覆盖率 | 把"没返回"读成"不存在"；分页到底也补不回来 |
| §八.3 | GSC 单飞键跨工具共用 `tools:gsc:inflight:<ip>` | 配额按 GCP project 计，一个 IP 同时跑两个工具就花掉其他访客的额度 |

另确认一条已知非缺陷：`public-tool-request.ts:81-102` 的单飞是 per-isolate best-effort（源码注释已声明），对外文案不得描述为硬限额。

---

*v3.1，2026-07-31。§一/§二.4/§八.3 于 2026-08-03 按第二轮 codex 评审回填（附录 C），M2 实施基线。*
