...
- **站点卡**（照 opengengrowth `Sidebar.tsx` 的四行：域名 / 市场 / GSC / 审计）：域名 = `ProjectShellProject.host`；市场 = 主站点 `market_codes[0]`（`ProjectShellProject` 增加 `marketCode` 字段，从 `SiteRow` 取，总是真实存在）；GSC = 真实连接状态（**PR-3 落地 `e048be6a`**：`ShellChrome` 在客户端用 `useProjectSources(projectId)`，经纯函数 `shell/gsc-connection.ts` 的 `gscConnectionState` 得出 `true / false / null`；server 侧 `WorkbenchShell` 只传 `Omit<SidebarSite, "gscConnected">`。「已接入」判据沿用仓库既有的 `id !== null && state !== "disconnected"`，`permission_denied` / `unavailable` 仍算已接入（计划 Q3，「需要重连」的真相留给数据源页）；loading、任何请求失败、响应没有 gsc 槽位、未 hydrate 一律 `null`，显示「—」并带 title `shell.siteCard.unknownHint`「现在读不到连接状态。未知不等于未接入。」（Q4）；判据按列举式写，矛盾或未知的状态不下结论（`9023a939`、`059569b3`）。不走服务端的理由见计划 Q2）；审计 = store 里 `lastAudit.at`（本地 mock 结果），有值时紧跟一个「示例」小标（`shell.sampleData`），没有则「—」；站点卡不放 GA4 行（照 opengengrowth 四行）。**真实与 mock 分开标注**（评审 F12）：前三行是真数据不标，第四行标「示例」。**PR-3 修订（Q6）**：GSC 行的「示例」标注跟着数据来源走，不跟 `state.demo`——数据源页行表只在 `gscRowsSource === "sample"` 时带「示例」chip（`GscRowsTable.tsx`），用户自己导入的行不标；侧栏的数据源徽标是纯行数，不标示例（计划 Q6 ④，注释写在 `shell/workbench-nav.ts` 的 `dataSources` 条目上方，`acb0e243`）。
- **顶栏**：复用现有 `ProjectSwitcher`；「＋ 新建站点」→ `/new-project`（这个链接也过 Context 离开确认，见下一条）；「搜索 / 跳转 ⌘K」按钮（打开命令面板，关闭后焦点回到它）；保留 `LocaleSwitch` 与 `signOutAction` 账号菜单；「示例数据」chip（`demo` 为真时显示「示例站点」并带「清除示例」——**PR-3 已落地 `3f058f8c`**，见 §6.7；移动端是 44×44 图标按钮、`md` 起为 26px 文字按钮；按钮持有焦点时被跨标签移除，焦点交给产物筐按钮（`62d3e25c`、`shell/useClearSample.ts`））。**chip 的范围（PR-3 Step 1b）**：chip 无条件显示，依据是审计、可见度、产物等模块结果无论 `demo` 与否都是本地模拟；它不覆盖用户自己导入的 GSC 行，GSC 行的来源由 `gscRowsSource` 单独标注（§6.8）。chip 的 title 写明这层范围（`shell.sampleTitle`，`ui/DemoChip.tsx`，`23c0fe53`）：「审计、可见度等模块结果是本地生成的示例，不包括你自己导入的 GSC 行。示例内容目前仅有中文。」；与之同屏的概览脚注 `overview.gscFoot.user` 补了限定：「这些 GSC 行来自你导入的数据，不是示例」。title 不按挂载点传参，所有挂载点共用这一句。DemoChip 的挂载点：顶栏、概览页头（恒为「示例数据」）、本周与档案页头、档案文档的爬取 / 第三方 / 示例来源 GSC 小节、占位页；设置页通知偏好块不挂（`77dafb0f`：偏好是用户自己的开关，不是示例结果）。title 只能悬停看到，触屏看不到，所以与用户导入的 GSC 数据同屏的地方另有可见脚注，复用 `overview.gscFoot.*` 这一句（`9dc7c575`，交付前验收「分区」F1）：本周临界词面板按当前 `gscRowsSource`（user / sample 各一句，null 不显示），档案 GSC 小节按快照 `gscSource`（只有 user 加脚注，sample 已有 DemoChip、null 已有「来源未知」）；「产物筐 N」开抽屉；storage 不可用时的「本次结果不会保存」提示（§6.5；`swept` 态静默）。这条提示只有**一个** `role="status"` 容器，始终渲染以保住 live region，整句由它播报；可见副本都是 `aria-hidden`：`<md` 与 `lg–xl` 显示紧凑文字，`md–lg`（768–1023）只显示 16px 琥珀色图标（非文本对比度实测 6.84:1），两者都带整句 title；`xl` 起整句可见，状态组可收缩、整句截断并带 title（`bb8eadbc`）。已知取舍：竖屏平板在存储失败时视觉上只有图标和悬停提示。
- **顶栏两行布局（PR-3 `4af1ef6c`、`bb8eadbc`，方案 Z）**：`<xl`（<1280px）顶栏分两行，DOM 顺序与单行时相同，只是按顺序切进两个明确的行容器（不靠 flex-wrap、不用 CSS order），所以 DOM、视觉、Tab 顺序在两种布局下一致（WCAG 1.3.2 / 2.4.3）。第一行：汉堡（`<md`）、项目切换器、「+ 新建站点」、搜索（`md+`）、存储提示、示例 chip（全文，`shrink-0`）；第二行右对齐：清除示例、产物筐、账户组，第二行 `shrink-0`。`xl` 起第一行容器 `display: contents`、第二行保留尾组容器，几何与修复前单行相同。缺陷来源：示例态 390px 时 chip 盖住汉堡（移动端打不开导航），768–1024px 横向溢出 17px，单行在 1024 就放不下（T17 实测）。宽度规则：`<xl` 只有切换器可收缩；`xl` 起存储整句先让位，切换器停在 `.wb-reset` 覆盖块里的 64px 下限（实测左组的内容最小宽度已托住 ≥72px，这条下限是第二道线）。切换器在工作台里的覆盖只写在 `app-shell.module.css` 的 `:global(.wb-reset) .projectSwitcher` 块：`min-width` 不再是 128px、`overflow: hidden`、`min-height: 0`（≥961 不再上下各探出行高 1px，`b8915323`）、`<768` 高 44px；`/new-project` 的 AppShell 保持 40px（≤960）/ 58px。切换器的 `select` 透明度 0.001，原来画在它上面的 outline 从来不可见，改为父盒 `:has(.projectSelect:focus-visible)` 的 inset box-shadow（`b8915323`）。几何、Tab 顺序、焦点环、44px 与存储失败态由 `e2e/workbench-pr3-flow.mock.spec.ts` 与 `workbench-shell.mock.spec.ts` 钉住。
- **命令面板 / 产物筐抽屉**：都是 `role="dialog" aria-modal="true"`，有 `aria-labelledby`；打开时焦点进入（面板进搜索框、抽屉进关闭按钮），关闭时焦点回到触发按钮；焦点圈在对话框内（Tab 循环）。**PR-3 交付前 a11y 验收后的回焦与候选规则**：
  - **关闭时依次尝试**：`returnFocusTo` → 打开时的焦点元素（`body` / `html` 不算）→ 调用方的 `fallbackFocus` → 工作台 `<main data-wb-main tabIndex={-1}>`。
  - **每一步都校验**：`isConnected`；不在 `[inert]` / `[aria-hidden="true"]` 子树；`focus()` 后回读 `activeElement` 确认接住（`72c5792f`、`7caf8cd9`）。触发按钮随确认动作卸载的两个入口（清除示例、清空 GSC 行）在 `flushSync` 前就改指局部目标（产物按钮、粘贴框）。
  - **`<main>` 的焦点环**：`globals.css` 按 `[data-wb-main]` 去掉全局焦点环（`49b1f269`），因为不在 Tab 顺序里的地标容器不需要焦点指示。鼠标点正文空白处也会让 `<main>` 成为 `activeElement`；工作台里没有依赖失焦或「焦点在 body」的判断。
  - **焦点候选**：初始聚焦和 Tab 循环共用同一份实时过滤的候选，排除 `hidden`、`[inert]`、`:disabled`、面板有盒子而自身无盒子的元素；`focus()` 失败就试下一个，全部失败则聚焦面板（`4f491a57`）。
  - **已知限制**：
    - 下层 Dialog 上再叠 ConfirmDialog 时，关闭上层不向下层回焦。今天不可达；产物筐清空若采纳加确认框的方案（Owner 候选 A），先修这一条。
    - Dialog 不锁背景滚动。
    - ConfirmDialog 的正文没有 `aria-describedby`。

  其余照旧：背景 `inert`；Esc 关闭；⌘K / Ctrl+K 切换面板。面板列表项是 `role="option"` 的 **`<Link>` 锚点**（不是按钮、更不是可点 div），Enter 等价于对高亮那一项的一次真实点击（`element.click()`），面板自己**不调 `router.push`**：Studio 编辑器的未保存守卫（`_unsaved-navigation-guard.ts`）是在 `document` 捕获阶段拦 `a[href]` 点击的，按钮驱动的 router push 会从它旁边走过去、把脏编辑静默丢掉（PR-1 验收红队 A1）。Context 离开确认在锚点自己的 `onClick` 里跑，同样靠 `preventDefault` 取消；被取消的点击 `Link` 不会导航，面板保持打开。**⌘K / 顶栏按钮在别的模态拥有页面时拒绝打开**（红队 A2）：旧页的 Product Profile 编辑器与 action override 会把 `document.body` 的每个子节点（含 `#wb-app`）设成 `inert` 并压在 z-index 1200；「根是 inert 且没有工作台面板开着」就是这个签名，此时 `ShellChrome` 的三个 opener 都原样返回，z-50 的对话框不会挂到那层遮罩底下抢焦点。配套地，`Dialog` 在 0→1 时快照根上**已有**的 `inert`、回到 0 时只摘自己设的那份（红队 A3），关闭时的焦点归还（首选目标与 opener 兜底都一样）跳过落在 `[inert]` / `[aria-hidden="true"]` 子树里的目标。一个布尔快照够用的前提是两类模态**不会交错**：我们的对话框开着时旧页已 inert、开不出它的模态；旧页模态开着时 `ShellChrome` 拒绝开我们的——可达的顺序只有严格嵌套（gpt-6-astra 复审提出的「先开我们的再开旧页模态」交错序列不可达，记录不修）。
...
| 文案裁决批（区间写法、`n/a`、chip title、空态） | S-copy 1（P3） | `8654235e` 四句无参数诚实性文案按 en / zh 整串相等 |
| Q35 `ui/` 不得 import `shell/` | S-pins 3（1 P2）→ S-pins r2 0 | `37ae84c2` 直接边规则、`6d2e4e71` 非字面量加载判失败 + 按后缀认测试文件 + 经真实遍历的目录树控制、`b7e852a7` 拆文件 |
| 顶栏两行布局（T17 实测发现的生产缺陷） | S-topbar 3（2 P2）→ S-topbar r2 2（已由 shell spec 覆盖，不修） | `4af1ef6c` 方案 Z、`b8915323` 1px 探出与焦点环、`bb8eadbc` 存储失败态（整句截断、768–1023 仅图标、`<768` 44px） |
| mock e2e 断言空洞 | S-e2e 6（5 P2）→ S-e2e r2 0 | `ac03bd88` Q34 非空 id、Q30 两态、四卡字面值、根自身 style；`e08ae173` 与 `67a7d8ad` profileDoc 快照与标记；GSC 行 `toHaveCount(0)` |
| 交付前：真实与示例分区 | S7-partition 1（P2 降 P3：页头 chip 标模块结果是既有裁决，title 触屏不可见） | `9dc7c575` 本周临界词面板与档案 GSC 小节的可见来源脚注 |
| 交付前：概览与本周诚实性 | S7-honesty 4（3 P2）→ weekfix r2 3（P3） | `3a64da98` 三句只说能证明的事、`d9522ed1` 时钟按分钟与可见性刷新、`6b3f3df5` 三句整串钉、`ba478507` 脚注测试构造 demo 相反组合 |
| 交付前：a11y 与 CSP | S7-a11y 5（4 P2；#1 叠层回焦不可达未修，记残留）→ a11y r2 3（1 P2） | `72c5792f` 回焦校验与回退到 `<main>`、`4f491a57` 焦点候选过滤、`386b2952` / `0f1c78b7` Tabs 失效值不再宣称选中、`7caf8cd9` body 不算 opener、`49b1f269` `<main>` 无焦点环 |

同期 Claude 变异审阅另有 store 3 条、T3 7 条、T6+T7 10 条（1 P1）、T12 6 条，以及 T9 与 T10 两轮（与对应跨模型面交叉）；两路盲区不同：Claude 一路靠变异，抓「测试守不住」（钉子形状、两端同源）；跨模型一路靠推状态组合，抓「代码在某个输入下说假话」。PR-3 交付前的整体跨模型验收（计划 T19 Step 7 的三面：诚实性、真实与示例分区、a11y 与 CSP）已于 2026-09-14 执行，发现与修复见上表后三行；每一轮修复后都再做一轮复核，直到 0 条或只剩已覆盖、不可达的条目。T17 的 38 条 e2e 变异中，除基线外全部变红：`switcher-floor-0` 是无效变异，64px 下限被左组的内容最小宽度先托住；F3 原本是断言比不出差异，补标记后变红。
