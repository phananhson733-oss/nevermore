...
状态：已落地并经四轮跨模型评审（契约 `daily_search_briefing.v3`），未 push
...
codex 四轮共抓出 7 高危 + 15 中低危。**前三轮每轮都在「我刚修好」的代码里抓到东西**。
...
第三轮那条是我改文案时把「两个窗口的页面归因都要可用」丢了——为了把条件性规则写准，
把无条件规则删掉了。

第四轮才收敛：两条 round-3 findings 判 FIXED，新发现的是**可解释性缺口**而非谎言——
点击机会的前提少说了三道门（非品牌词 / 当前窗口 ≥100 / 剔除自己后该段仍能算基线），
以及「形成候选之后还有三条呈现规则」（同词只显示一条 / 三行上限 / 让出交接名额）
在页面上无处可查。都已补上，`rowAccounting.notSelectedVisibleRows` 也终于渲染了
——它从 v3 起就在契约里，一直没接线（同 [[parsed-and-validated-is-not-used]]）。

**什么时候可以停**：当一轮评审的产出从「这句是假的」变成「这句不够完整」时。
