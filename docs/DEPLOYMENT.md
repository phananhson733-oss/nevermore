...
Migration range: `0001_init.sql` through `0048_topic_model_generation.sql` (**48 ordered migrations**); `0047_dataforseo_competitor_metrics.sql` immediately precedes the Topic migration.

Historical production evidence through `0021` does not prove that the active
v0.4 migrations through `0048` are hosted; every release must back up,
