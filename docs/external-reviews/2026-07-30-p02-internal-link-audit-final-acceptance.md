...
The following checks passed in an isolated worktree created from the current
`origin/main` and containing only the P0-2 commit:
...
- Root `pnpm typecheck`
- Root `pnpm lint`
- Root `pnpm test` — 488 files and 5,990 tests passed
- Root `pnpm build`
- `pnpm verify:spec`
- `pnpm secrets:scan` — scan and 75 redaction tests passed
- `git diff --check`

The source working branch also passed the combined P0-2/P0-4 marketing E2E
(5 tests) and its in-progress `pnpm verify:public-tools-boundary` check (88
traced runtime files). The current `origin/main` does not yet contain that
boundary script, so it was not claimed as an isolated-main gate. Static source
inspection confirms the P0-2 route imports only marketing components and its
page-local fixed demo data; it does not import authenticated app, worker,
...
The P0-2 changes were replayed onto the current `origin/main` baseline
`9d992194c299c69606d2cd79336b7e7eda949120`, verified in an isolated worktree,
committed as a dedicated P0-2 change and pushed to
`phananhson733-oss/nevermore` `main`.

The original working tree still contains pre-existing P0-4 and unrelated
application changes; none of those dirty-tree changes were included in the
isolated main commit. No deployment, database migration or production
