...
- [x] **Step 2: 解 `agent-results.test.tsx` 的冲突**
...
- [x] **Step 5: 提交（cherry-pick 已各自成提交，只需确认历史）**
...
- [x] **Step 1: 写失败测试**：三种严重度各渲染出 `P0 · 阻断项` / `P1 · 警告` / `P2 · 建议`；一个 `result` 为未知字符串的检查**不出现任何 P 徽章**且进隔离区。
- [x] **Step 2: 跑，确认红**
- [x] **Step 3: 实现**：从 `agent-result-helpers.ts` 导出 `RESULT_PRIORITY`，accordion 引用它；隔离分支在徽章之前 return。
- [x] **Step 4: 跑，确认绿**
- [x] **Step 5: 提交**
...
- [x] **Step 1: 写失败测试**：默认渲染下「未采集」「最终源站」不可见（在 details 内、未展开）；一行事实文本出现；四个计数芯片可点击且是筛选器。
- [x] **Step 2: 跑，确认红**
- [x] **Step 3: 实现**
- [x] **Step 4: 跑，确认绿**
- [x] **Step 5: 提交**
...
- [x] **Step 1: grep 引用面**：`git grep -n "AgentDiagnosis\|agent-diagnosis"`
- [x] **Step 2: 写失败测试**：结果面不再渲染范围切换按钮（`站级` / `页面级`）与组导航。
- [x] **Step 3: 跑，确认红**
- [x] **Step 4: 删除 + 改引用**
- [x] **Step 5: 跑全量 agents 单测 + typecheck，确认绿**
- [x] **Step 6: 提交**
...
- [x] **Step 1: grep 旧文案的所有落点**（字典 + metadata + JSON-LD 硬编码源，见设计 §7「文案诚实性」）
- [x] **Step 2: 改文案，跑 key parity 与「渲染输出不含 key 路径前缀」断言**
- [x] **Step 3: 构建后跑 e2e**
...
- [x] **Step 1: 写失败测试**（候选选择）：非 2xx / 非 HTML 剔除；`subjectUrl` 去重保留第一条；首页第一、目标 URL 第二；深度 1 按 `inboundLinks` 降序、不足取深度 2；上限 24；`pages` 为空 → `[]`。
- [x] **Step 2: 写失败测试**（URL 形态）：用真实报告的 `pages[].url` 构造候选，断言 `projectRecordToTarget` 能匹配上；把候选换成 `subjectUrl` 必须让该断言变红。
- [x] **Step 3: 写失败测试**（wire 守卫）：多字段 / 少字段 / 超长 / 负数各一条拒绝；缺 `keyPages` 放行。
- [x] **Step 4: 跑，确认红**
- [x] **Step 5: 实现**
- [x] **Step 6: 跑，确认绿**
- [x] **Step 7: 提交**
...
- [x] **Step 1: 写失败测试**：coreFeatures 为空 → 按候选原序、整组 `basis = "structure"`；首页即目标不重复；候选不足 12；整词命中 +1000、单词 +1；CJK feature；同分稳定序；`basis` 四种取值正确。
- [x] **Step 2: 跑，确认红**
- [x] **Step 3: 实现**
- [x] **Step 4: 跑，确认绿**
- [x] **Step 5: 提交**
...
- [x] **Step 1: 写失败测试**：「目标页有 title、关键页无 title」的 fixture 下，2.1 在该关键页**不得为 pass**（必须 excluded）；`target_query_ranking_band` 在非目标页被剔除（不走 precondition limitation）；`every_collected_page` 记录在非目标页仍可 pass；`targetInspected = false` 时 8.x/9.x 仍可判定。
- [x] **Step 2: 跑，确认红**
- [x] **Step 3: 实现**
- [x] **Step 4: 跑，确认绿**
- [x] **Step 5: 提交**
...
- [x] **Step 1: 写失败测试**：全通过 / 全排除 / 全来源受限 / 混合 / 未知枚举 fail-closed（且隔离先于 P2 兜底）/ 关键页命中数 > 全站受影响数触发隔离 / `mode = "not-captured"` 出现即隔离 / `declaresNoJudgement` 项在任何分布下都不得合并成 pass。
- [x] **Step 2: 变异测试**：把「取最差」改成「取最好」必须至少一条红（写在测试注释里，人工验一次）。
- [x] **Step 3: 跑，确认红**
- [x] **Step 4: 实现**
- [x] **Step 5: 跑，确认绿**
- [x] **Step 6: 提交**
...
- [x] **Step 1: 写失败测试**：两种标签格式；`enumerated = false` 时写「另有至少」；站点级四字段为 0 且不列 URL；来源受限 `totalCount` 为 null 不为 0；`keyPageHitCount > totalCount` 触发隔离。
- [x] **Step 2: 跑，确认红**
- [x] **Step 3: 实现**
- [x] **Step 4: 跑，确认绿**
- [x] **Step 5: 空态与退化**：`keyPages` 缺失/为空时写「本次没有可评估的关键页，页面级结论只针对目标 URL」并退化为单目标。
- [x] **Step 6: 提交**
...
- [x] **Step 1: 写失败测试**：每条记录的构造器 × `isSeoAuditRecord` sweep；含「全站合规」干净分支（→ `not_observed`，`affected = 0`，绝不 `observed + affected 0`）与「缺侧车」分支。
- [x] **Step 2: 跑，确认红**
- [x] **Step 3: 实现 + 登记台账**
- [x] **Step 4: 跑，确认绿；`detector-contract.test.ts` 双向台账比对通过**
- [x] **Step 5: 提交**
...
- [x] **Step 1: 写失败测试**：`AGENT_AUDIT_COVERAGE.total` 从 ids 派生为 **89**（不手写数字，从 `SITE_CHECK_IDS + PAGE_CHECK_IDS` 推）；九个新 id 在集合内；每条有专属 `howToFix`（不与同组兄弟共用）。
- [x] **Step 2: 跑，确认红**
- [x] **Step 3: 实现**
- [x] **Step 4: 跑，确认绿**
- [x] **Step 5: 提交**
...
- [x] **Step 1: 写失败测试**：三处版本字面一致的冻结断言；Checker 与 catalog 引用同一常量导出（改一处必须让两边都变）；文本/代码比不再扣分。
- [x] **Step 2: 跑，确认红**
- [x] **Step 3: 实现**
- [x] **Step 4: 跑，确认绿；`client-bundle-boundary.test.ts` 仍绿（新子路径不是 barrel）**
- [x] **Step 5: 950 页 fixture 实测 payload 体积，记录到 PR 描述（设计 §13）**
- [x] **Step 6: 提交**
...
- [x] **Step 1: 写失败测试**：有 `pageRole` 无 `targetQueries` 时 `pageShapeChecks` 出现且 3.4/3.5/3.6/7.2 可判定；两者都无时缺席；`isAgentKeywordChecks` 对搬迁后的 id 集合仍通过。
- [x] **Step 2: 跑，确认红** → **Step 3: 实现** → **Step 4: 跑，确认绿** → **Step 5: 提交**
...
- [x] **Step 1: 写失败测试**：10 项在任何分布下都不是 Tip、不是 pass、不进建议；`measurement()` 对观测记录展示数值。
- [x] **Step 2: 跑，确认红** → **Step 3: 实现** → **Step 4: 跑，确认绿** → **Step 5: 提交**
...
- [x] **Step 1: 写失败测试**：纯中文页在 3.6 上不判 thin。
- [x] **Step 2–5: 红 → 实现 → 绿 → 提交**
...
- [x] **Step 1: 写失败测试**：超宽草稿被标出而不被拒绝；含词复核生效。
- [x] **Step 2–5: 红 → 实现 → 绿 → 提交**
...
- [x] **Task 5.1: 全量单测 + typecheck + lint + secrets 扫描**
- [x] **Task 5.2: `pnpm --filter @sf/marketing build` 后跑 e2e（双 locale、390px、干净站点空态、关键页行）**
- [x] **Task 5.3: 文案清扫** —— 字典 + metadata + JSON-LD 硬编码源三处都要扫（教训：只扫字典会漏掉搜索结果里显示的那段）
