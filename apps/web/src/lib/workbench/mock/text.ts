...
const ALPHABET_DIACRITICS = /(?<=[\p{Script=Latin}\p{Script=Greek}\p{Script=Cyrillic}])\p{M}+/gu;
...
export const COMPETITOR_PLACEHOLDERS = ["[竞品 A]", "[竞品 B]", "[竞品 C]"] as const;

/** URL path segment for any script, at most 60 code points; `q-<hash>` when nothing letter-like survives. */
export function slugify(value: string): string {
  const dashed = value
    ...
    .replace(EDGE_DASHES, "");
  const cut = Array.from(dashed).slice(0, SLUG_MAX_CODE_POINTS).join("").replace(EDGE_DASHES, "");
  return cut.length > 0 ? cut : `q-${hashOf(value).toString(36)}`;
}
...
export function domainOf(url: string): string {
  ...
  if (trimmed === "") return "";
  try {
    const { hostname } = new URL(HAS_SCHEME.test(trimmed) ? trimmed : `https://${trimmed}`);
    return hostname.replace(/^www\./, "").replace(/\.$/, "");
  } catch {
    // Not a parseable URL (e.g. a space in the host): best-effort slice, never throw.
    return trimmed.replace(HAS_SCHEME, "").replace(/^www\./i, "").split("/")[0] ?? "";
  }
}
...
export function matchesBrand(text: string, brand: string): boolean {
  ...
  const wholeWord = new RegExp(`(?<![\\p{L}\\p{N}])${escapeRegExp(wanted)}(?![\\p{L}\\p{N}])`, "u");
...
export function competitorNames(profile: Pick<Profile, "competitors">): readonly string[] {
  ...
  const unique = names.filter((_, index) => keys.indexOf(keys[index] ?? "") === index);
