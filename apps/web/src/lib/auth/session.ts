...
async function getAuthUser(): Promise<{ userId: string; displayName: string } | null> {
 ...
 */
async function ensureOperator(userId: string, displayName: string): Promise<OperatorContext> {
  const { db } = getDb();
  ...
  return db.transaction(async (tx) => {
    await tx.execute(sql`select pg_advisory_xact_lock(hashtext('sf_singleton_bootstrap'))`);
    ...
    if (!workspaceRow) {
      workspaceRow = (
        await tx.insert(workspaces).values({ name: SINGLETON_WORKSPACE_NAME }).returning({
          id: workspaces.id,
        })
      )[0];
    }
    ...
    await tx
      ...
      .values({ user_id: userId, workspace_id: workspaceId, display_name: displayName })
      .onConflictDoNothing();
