...
describe("StatCard", () => {
  it("prints an em dash for an unknown value and no zero anywhere", () => {
    const scope = render(<StatCard value={null} label="Health" foot="No audit run yet" />);
  ...
  it("does not accent an unknown value", () => {
    ...
    const scope = render(
      <StatCard value={null} label="AI mentions" foot="No check run yet" accent="fuchsia" />,
    );
  ...
  it("prints a measured zero as a zero", () => {
    const scope = render(<StatCard value="0" label="Artifacts" foot="This session" />);
  ...
  it("uses the rail's GEO tone for the AI-mention accent, never violet", () => {
    ...
    const scope = render(
      <StatCard value="35%" label="AI mentions" foot="14 of 40 answers" accent="fuchsia" />,
    );
    const value = scope.querySelector("span");
  ...
  it("renders the delta when one is given and nothing when it is not", () => {
    const scope = render(<StatCard value="56" label="Health" foot="9 findings" delta={7} />);

    expect(scope.textContent).toContain("+7");
    cleanup?.();

    const bare = render(<StatCard value="56" label="Health" foot="9 findings" />);
    expect(bare.textContent).not.toContain("+");
  });

  it("keeps the label and the footnote the caller passed", () => {
    const scope = render(
      <StatCard value="12" label="Keyword candidates" foot={<span>8 from GSC</span>} />,
    );
  ...
  it("becomes one anchor over the whole card when a href is given", () => {
    const scope = render(
      <StatCard value="5" label="Borderline" foot="Positions 11-30" href="/p/abc/keywords" />,
    );
  ...
  it("is not a link when no href is given", () => {
    const scope = render(<StatCard value="5" label="Borderline" foot="Positions 11-30" />);
  ...
  it("carries no inline style (production CSP has no unsafe-inline)", () => {
    const scope = render(
      <StatCard value="56" label="Health" foot="9 findings" delta={-2} href="/p/abc/audit" />,
    );
