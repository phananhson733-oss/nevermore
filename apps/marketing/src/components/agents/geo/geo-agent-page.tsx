...
import { Check, Database, LockKeyhole, Radar } from "lucide-react";
...
import { GeoWorkbench } from "./geo-workbench";

export const GEO_AGENT_PATH = "/agents/geo";

export async function GeoAgentPage({ locale }: { readonly locale: string }) {
  const [t, messages] = await Promise.all([
    getTranslations({ locale, namespace: "agents.geo" }),
    getMessages(),
  ]);
      ...
      <div className="mx-auto max-w-report px-6 md:px-8">
        <BreadcrumbJsonLd
          items={[
            ...
            { name: t("name"), url: localeUrl(locale, GEO_AGENT_PATH) },
          ]}
        />
        <VisibleBreadcrumb
          items={[
