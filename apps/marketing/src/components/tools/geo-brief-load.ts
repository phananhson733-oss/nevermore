...
export interface FrozenQuestion { readonly id: string; readonly text: string; readonly layer: string; readonly roleId: string | null; readonly role: FrozenRole | null }
...
export function parseLoadedBriefChoices(value: unknown): LoadedBriefChoices | null {
  ...
  for (const value of row.choices) {
    ...
    for (const value of choice.questions) {
      ...
      questions.push({ id: q.id, text: q.text, layer: q.layer, roleId: q.roleId, role });
    }
    choices.push({
