...
- `content-draft-v2-results.tsx` — coverage-first quality assessment separate from processing status, collapsible H2/H3 prose and rerun controls, distinct source-tier/claim labels and verify list; one source line per paragraph with the per-sentence annotations off by default; no published-URL exit, since the tool publishes nothing; exact JSON/Markdown with one confirmed related-links block; export receipts bind immutable result, confirmation, locale and latest action, not only the causal fingerprint.
