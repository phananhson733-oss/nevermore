...
// @pos -- browser-local confirmation/export boundary; no storage, provider calls or navigation
"use client";
...
export function ContentBriefV2Editor({ brief, onConfirmed, children }: {
  ...
  useEffect(() => {
    ...
    return () => { state.mounted = false; state.generation += 1; state.inFlight = false; callback.current?.(null); };
  }, []);
  ...
  function invalidate() {
    ...
    setPending(false); setConfirmed(null); setError(null); setExportStatus(null); setCopying(false);
    callback.current?.(null);
  }
