...
describe("GEO shared Brief producer", () => {
  it("carries only provenance-backed Profile values into factual receipts", () => {
    const context: GeoSnapshotContext = { schemaVersion: "marketing-geo-snapshot-context.v1", kbId: SHARED_FROZEN.kbId, targetHost: "fixture.example", payloadHash: SHARED_FROZEN.contentHash, questionSetHash: SHARED_FROZEN.questionSetHash, contentHash: "c".repeat(64), enrichment: null, roles: [], facts: SHARED_FROZEN.payload.facts.map(fact => ({ key: fact.key, value: fact.value || null, reason: fact.reason, source: "kb" as const, sourceUrl: fact.sourceUrl || null, observedAt: fact.observedAt || null, evidenceId: null })), competitors: [], skippedLayers: [], profile: { reference: { schemaVersion: "website-profile-reference.v1", websiteId: "fixture-website", snapshotId: "fixture-profile", snapshotRevision: 1, profileSchemaVersion: "marketing-website-profile.v1", profileHash: "d".repeat(64) }, productName: "Fixture product", oneLinePositioning: "Inferred superiority claim", coreFeatures: ["Declared feature"], market: { country: "US", language: "en" }, fieldProvenance: ["productName", "oneLinePositioning", "coreFeatures"].map(field => ({ path: `/${field}` as "/productName" | "/oneLinePositioning" | "/coreFeatures", derivation: field === "oneLinePositioning" ? "inferred" : "declared", confidence: "high", source: "user_edit", limitation: null, observedAt: null, evidenceUrls: [] })) } };
    const brief = sharedGeoBriefBasis({ frozen: SHARED_FROZEN, context, knowledgePack: null, questionId: "q1", questionText: "", runEvidence: null, runId: "fixture-brief", now: "2026-08-31T00:00:01Z" });
    expect(brief.evidence.facts.map(fact => [fact.source, fact.text])).toEqual([["kb", "Three seats"], ["product_profile", "Fixture product"], ["product_profile", "Declared feature"]]);
    ...
    const preserved = sharedGeoBriefBasis({ frozen: SHARED_FROZEN, context, knowledgePack: null, questionId: "q1", questionText: "", runEvidence: null, runId: "fixture-brief", now: "2026-08-31T00:00:01Z" });
  ...
  it("produces a valid frozen/manual Brief with KB Q1 and no invented sample evidence", async () => {
    const basis = sharedGeoBriefBasis({ frozen: SHARED_FROZEN, context: null, knowledgePack: null, questionId: "q1", questionText: "CLIENT OVERRIDE", runEvidence: null, runId: "brief-1", now: "2026-08-31T00:00:01Z" });
  ...
  it("marks typed manual Q1 user_input with no frozen question, role, entities or sample", () => {
    const brief = sharedGeoBriefBasis({ frozen: SHARED_FROZEN, context: null, knowledgePack: null, questionId: null, questionText: "A custom question?", runEvidence: null, runId: "brief-1", now: "2026-08-31T00:00:01Z" });
