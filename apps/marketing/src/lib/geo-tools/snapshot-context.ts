...
import { z } from "zod";
import { normalizeAccountWebsiteUrl, parseWebsiteProfileReference, fieldProvenanceSchema, WEBSITE_PROFILE_LIST_MAX_ITEMS } from "../account-websites/contracts.ts";
...
const contextSchema = z.object({
  ...
  profile: z.object({ reference: profileReference, productName: text, oneLinePositioning: text, coreFeatures: z.array(text).max(WEBSITE_PROFILE_LIST_MAX_ITEMS).readonly(), market: z.object({ country: text, language: text }).strict(), fieldProvenance: z.array(fieldProvenanceSchema).max(3).refine((rows) => rows.every((row) => ["/productName", "/oneLinePositioning", "/coreFeatures"].includes(row.path)) && new Set(rows.map((row) => row.path)).size === rows.length).readonly().optional() }).strict().nullable(),
