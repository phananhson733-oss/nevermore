...
format, gap_angle and the plan have to agree. On create the outline is the whole page, so do not choose a format the outline does not carry out: a tool whose outline never covers using it and reading its result is a guide, whatever the sample says. On update the format describes the page that already exists, not the edits, and the steps are the plan. In both, do not promise work in gap_angle that the plan does not contain. When the evidence cannot support the missing part, change the format or narrow the angle; never add a section or step the evidence does not support, and never invent a procedure.
