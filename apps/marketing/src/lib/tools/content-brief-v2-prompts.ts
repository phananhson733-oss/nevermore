...
format, gap_angle and the outline are one plan and have to agree: on create the outline is the entire plan. Do not choose a format whose reader task the plan does not carry out -- a tool whose outline never covers using it and reading its result is a guide, whatever the sample says. Every distinct promise in gap_angle needs a question or section that carries it. When the evidence cannot support the missing part, change the format or narrow the angle; never add a section the evidence does not support or invent a procedure.
