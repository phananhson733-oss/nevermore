...
The failure to watch for is the export that silently truncated. Search Console caps a UI export at 1,000 rows, so a large blog will hand you a file where the tail of the site simply is not present. If you paste that file and say nothing, every missing page looks like a page with no impressions, and the audit will retire a chunk of your site on evidence that does not exist. Mark those rows "not in export" and fill in `metrics_source` with the real period; the prompt then returns them as provisional with an instruction to check indexation before the verdict is acted on.
...
| /blog/what-is-dispatch-software | Keep | Provisional | Definition page for the owner in site context, and nothing in its row is shown to have expired. Absent from the export: a missing row can mean no impressions or an export truncated at the row limit, and those need different actions | Inspect the URL in Search Console, pull a page-filtered export for this path, then rerun this row | Stays live pending that check |
| /blog/hvac-technician-pay-guide | Keep | Provisional | Largest impression source at 96,400. It serves technicians rather than the owner in site context, but nothing here says what it produces | None from this audit. Trial signups assisted by this URL would settle it — a conversion figure this inventory does not carry | Stays live |
...
- No other group. The remaining seven pages hold distinct jobs.
