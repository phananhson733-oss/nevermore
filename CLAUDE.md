...
3. `authority/implementation-spec-v0.4/schema.sql`（由 `packages/db/migrations/0001_init.sql` 至 `0052_keyword_governance_schedule_requests.sql` 机械生成）— 当前 PostgreSQL 表、约束与索引的机器权威。
...
pnpm db:migrate            # 按序应用 0001–0052（幂等，第二次为 no-op）
