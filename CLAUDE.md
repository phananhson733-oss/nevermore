...
3. `authority/implementation-spec-v0.4/schema.sql`（由 `packages/db/migrations/0001_init.sql` 至 `0053_keyword_governance_suggestion_locale_authority.sql` 机械生成）— 当前 PostgreSQL 表、约束与索引的机器权威。
...
当前确定性版本为 `mvp.rules.0.2.4` / `mvp.prompts.0.2.0`，ordered migration head 为 `0053_keyword_governance_suggestion_locale_authority.sql`（53 个 migration）。`0042` 以 `NOT VALID` 在短事务锁窗口安装扩展后的 rule-set 约束，`0043` 再以较低级别锁验证历史行；`0044` 在不增加表或规则的前提下加入 DataForSEO Backlinks provenance、typed authority scale、selective crawler verification 与 Analysis Refresh v2/v1 兼容约束；`0045` 保留原始 Provider link identity，并允许同一站点 domain family 内的 DataForSEO target lineage，同时继续拒绝外域和带 credentials 的目标；`0046` 为 spec §1.6 的自助注册加上 workspace `plan_tier`，以 metadata-only 的 ADD COLUMN DEFAULT 回填既有行为 `internal`、再把默认值切到 `free`，全程零行改写；`0047` 加入 DataForSEO Search Landscape v3、canonical organic-overlap operand/ratio、fixed-20 AI citation aggregate 与 immutable competitor origin lineage，同时保留 v1/v2 只读兼容；`0048` 加入 bounded Topic Model generation child、invocation-attempt fence 与 Analysis Refresh v3，同时保留 v1/v2 exact historical readability；`0049` 在不改变这些 lineage authority 的前提下，把 Keyword/Competitor 投影和 provider discrepancy 收敛改为有界批量/集合式数据库写入；`0050` 为 fixed-20 GenerativeQuery Keyword cohort 增加显式 confirmed Product Profile lineage，且不伪造 provider provenance；`0051` 增加冻结输入、付费调用 fence、当前 authority CAS 与原子人工决议的 Keyword governance suggestion authority；`0052` 增加 payload-free durable source schedule request、lease-token dispatch CAS、generation continuation 原子请求与统一 stale-pending authority sweep；`0053` 以 forward-only routine replacement 把唯一 primary Site 的 singleton language 固定为 Keyword suggestion authority：server-owned freezer 以 `Intl.getCanonicalLocales` 产生 app-canonical manifest 并在 alias 改写时 fail closed，数据库 gate 仅对该可信 manifest 做 case-only Site identity/current authority CAS；Project delivery locale 仅控制交付文案且单独变化不得使建议 stale。Growth Audit 当前 read-model projection 是 `growth-audit.0.3.1`，但 capability version 仍为 `0.3.0`，request/addressing shape 与 `capabilityContractVersion` literal 仍为 `growth-audit.0.3.0`。
...
pnpm db:migrate            # 按序应用 0001–0053（幂等，第二次为 no-op）
pnpm db:migrate:check      # 断言 84 张 app 表 + 必需索引与 append-only trigger
...
- **Analysis Refresh / DataForSEO**：新 `createAnalysisRefreshRun` 冻结七步 `analysis-refresh.plan.v3`（Crawl → GSC → GA4 → DFS → `dataforseo_backlinks` → optional internal Topic Model generation → Growth Audit）；历史五步 `analysis-refresh.plan.v1` 与六步 `analysis-refresh.plan.v2` 只按各自 exact manifest/hash/ordinal 读取与恢复。Topic child 冻结 bounded input/resource ledger 和 invocation-attempt ledger，模型调用必须在数据库事务外，`reserved/outcome_unknown` 会阻断静默重试；它没有浏览器 create/reservation/provider-options API。DataForSEO Search Landscape（DFS）v3 从冻结 Site/market/language 与服务端 row cap 查询 positions 1–100，并仅在 canonical organic overlap 为空时使用带来源的 GSC/Crawl/Product Profile 种子追加一次 SERP Competitors fallback；v1/v2 继续精确只读兼容。v3 的 `DATAFORSEO_AI_CITATIONS_ENABLED` 付费子能力独立且默认关闭，只有同市场/语言中恰好 20 条 current approved、mapping-confirmed GenerativeQuery 与一个服务端固定 model 时才执行，19 条或 21-row overflow 都跳过且不发请求。DataForSEO Backlinks 以 `dataforseo.backlinks.v1` 原子写 provider Snapshot，authority metric 只能是 `dataforseo_rank`；cap 内 source page verification 复用 SSRF-safe、DNS/IP-pinned transport，且不改写 provider fact。独立 `DATAFORSEO_BACKLINKS_ENABLED` rollout 默认关闭；backlink/referring-domain/target-page/source-verification 默认 cap 为 500/100/500/20，硬上限为 1000/1000/1000/20。公开 `createCollectionRun` 只能触发 `crawl|gsc|ga4`，不得接受 DFS/Backlinks/Topic scope、limit、凭据、provider query 或 model option。
...
- **Keyword truth 与 content delivery**：DataForSEO ranked-keyword KD 只能是 `0..100` integer 或 `null`，provider search intent 只能是 canonical four-value taxonomy 或 `null`；present malformed value 必须 fail closed，不能把缺失值猜成 `0`。`searchIntent` 固定按 user-confirmed → exact provider-observed → invocation-backed LLM-generated → governed legacy → unavailable 解析，published pin 不得读取更新 lineage。Keyword delivery 只能从 complete inventories 连接 exact published-run mapped SitePage、拥有该页的 content Opportunity 与当前 content Artifact；Topic peer、Finding-only preview 和 technical output 不得冒充交付，Artifact 状态也不是 external publication authority。
- **Keyword suggestion locale authority**：actual BCP-47 canonicalization 只由 server-owned freezer 的 `Intl.getCanonicalLocales` 执行；它仅在 lowercase identity 不变时输出 canonical tag，所以 `en-us` 可冻结为 `en-US`，`iw-IL` 这类 alias 改写则在生成 manifest 前 fail closed。PostgreSQL 的 `app.is_bcp47_canonical_identity` 不是 canonicalizer；freeze 之后的 reuse、readiness、付费结果最终 TxB CAS、stale sweep 与 accepted approval 信任该 app-canonical manifest，只核唯一 primary Site 的 singleton spelling 与 manifest/Keyword tag 的 valid-shape、canonical casing、case-only identity 以及当前 market authority。浏览器 `anon/authenticated` role 无权创建 generation run 或写 canonical `app` 表；缺失、多值或 authority 漂移仍 fail closed。`client_projects.default_delivery_locale` 只控制客户交付/报告语言，不作 fallback，单独变化不得使建议 stale。
...
- **pg-boss 独立 schema（AC-004）**：`pgboss` schema 由库在 `startBoss()` 创建，绝不镜像进 Drizzle migration。84 张 app 表不含任何 pg-boss 表。
...
- **审计、代码核查、review 一律尽量多地拉 `gpt-6-astra`（reasoning effort `high`）**（Owner 2026-09-13 明确要求）：不要只留到收尾那一轮。凡是排查缺陷、方案/计划评审、PR 审计、上线前验收、"我已修好"的复核，**默认就该有一路跨模型**，与 Claude 审阅并行而不是二选一——两边的盲区不一样，历史上它反复在"我刚修好的代码"里抓到 P0/P1。具体落点至少包括：①计划/方案定稿前（只读计划与设计稿，找自相矛盾、不可达裁决、类型不可能）；②每个实现任务提交后的攻击面复核（按下面的边界切分，一次一个面）；③修复轮之后的复核（"改完了"是最容易骗过自查的状态）；④PR 交付前的整体验收。受限于单次用量，**同一轮并行的跨模型视角上限 2-3 个**（与"不要开几十个 subagent"同源），但轮数不设限——宁可分多轮小批量，也不要一次塞满然后零结论。调用方式：`mcp__codex__codex` 走不通时（本机全局 codex 不认 `gpt-6-astra`，报 requires a newer version），用 scratchpad 装最新 CLI —— `npm_config_cache=$SCRATCH/npm-cache npm i --prefix $SCRATCH/codex-local @openai/codex@latest`，再 `$SCRATCH/codex-local/node_modules/.bin/codex exec --sandbox read-only --skip-git-repo-check -C <worktree> - < prompt.txt > out.txt 2> err.txt`。三条硬性约束：**给边界**（把 diff 落成文件让它读，明令"不要广泛 grep 仓库"，最多指定四个上下文文件，否则它会读一万行然后被杀掉零结论；**但 diff 里每一处跨文件调用，被调函数的定义文件必须在清单里**——它守规矩不会去翻清单外的文件，2026-09-09 漏给了定义 `canonicalCrawlTargetKey` 的那 8 行，一条真实的 P1（重定向检查从「同源」放宽成「同主机名任意端口」）就只能被它写成「unresolved dependency question」而不是缺陷。要削减的是同类文件的数量和无关的大文件，不是依赖链上的那一环）；**控量**（一次只审一个攻击面 + 一到两个文件，单次约 10 万 token 能出结论，20 万 token 会撞用量上限且零发现）；**判据是 verdict 行**（输出里没有明确编号结论就是没跑成，如实说没跑成，不得把"已拉起"当成"已通过"；反过来 `0 findings` 也不等于没价值——**必须读它标为 unresolved / 无法确认的段落**，那里往往是它定位对了但手上证据不够的真缺陷）。stderr 里刷屏的 `missing field base_instructions` 不影响结果，`grep -v` 掉即可。
