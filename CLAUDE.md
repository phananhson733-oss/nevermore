...
pnpm verify:docs           # 文档版本、authority、79/10/78/11、四路由与发布边界一致性
pnpm verify:authority      # repository-owned active v0.4 authority 自校验
pnpm verify:spec           # v0.4 lock：79 operationId / 10 shared async / 78 表 / 11 规则 + 哈希一致性
...
- **仍在 v0.4 范围外**：RBAC/成员/席位/客户 Portal、Billing/pricing/subscription、Semrush API 深接、PDF/PPT/Word、公共 API/Webhook、单一“SEO 总分”或排名/收入保证、多 Workspace/硬删除、浏览器渲染 crawler、无证据的模型答案可见性监控，以及真正的 GitHub/WordPress publication-attempt external write。DataForSEO 继续受 feature flag、row cap、成本与证据诚实性约束。
...
- **Ahrefs API 永久禁止接入**（不是版本范围问题，也与账号权限无关）：不得调用、依赖、配置、鉴权、暴露凭据，或以任何方式提供指向 Ahrefs API / 网络集成 / MCP 工具的可执行路径。`ahrefs` 作为**惰性来源标记**仍然合法——provider 枚举成员、SQL CHECK 约束、OpenAPI 枚举、客户可见文案与注释都不受影响；被禁的是可执行接入面。由 `pnpm implementation:check` 里的 `checkAhrefsApiProhibition()` 强制，行为测试见 `scripts/ahrefs-prohibition.test.mjs`。**注意仓库门禁管不到全局环境**：本机或 agent 环境里全局安装的 Ahrefs MCP 工具需要在环境侧禁用，静态检查只能拦住提交进本仓库的配置与调用。
