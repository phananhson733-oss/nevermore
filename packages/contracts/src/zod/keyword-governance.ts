...
const TopicGenerationLimitation = z.enum([
...
export const TopicModelConfirmationMode = z.enum([
  "system_auto",
  "user",
  "legacy",
]);
export type TopicModelConfirmationMode = z.infer<
  typeof TopicModelConfirmationMode
>;

export const TopicModelGenerationSummary = z
  .object({
    origin: z.literal("llm_auto_confirmed"),
    generationVersion: z.literal("topic-model-generation.v1"),
    baseTopicModelRevision: z.null(),
    analysisInvocationId: Uuid,
    promptSetVersion: ClassificationLabel,
    inputHash: Sha256Hex,
    generatedAt: IsoDateTime,
    keywordGroupCount: NonNegativeRevision,
    keywordCount: NonNegativeRevision,
    assignedCount: NonNegativeRevision,
    unassignedGroupCount: NonNegativeRevision,
    skippedCount: NonNegativeRevision,
    limitations: z
      .array(TopicGenerationLimitation)
      .max(2)
      .refine(isUnique, "Generation limitations must be unique"),
    reason: z.literal("Initial model generated by Analysis Refresh"),
  })
  .strict()
  .superRefine((summary, ctx) => {
    if (summary.keywordGroupCount > summary.keywordCount) {
      ctx.addIssue({
        code: "custom",
        path: ["keywordGroupCount"],
        message: "Keyword group count cannot exceed keyword count",
      });
    }
    if (summary.assignedCount + summary.skippedCount !== summary.keywordCount) {
      ctx.addIssue({
        code: "custom",
        path: ["assignedCount"],
        message: "Assigned and skipped counts must cover the frozen keywords",
      });
    }
    if (summary.unassignedGroupCount > summary.keywordGroupCount) {
      ctx.addIssue({
        code: "custom",
        path: ["unassignedGroupCount"],
        message: "Unassigned groups cannot exceed the frozen groups",
      });
    }
    const expectedLimitations: Array<z.infer<typeof TopicGenerationLimitation>> = [];
    if (summary.skippedCount > 0) {
      expectedLimitations.push("keyword_assignments_skipped");
    }
    if (summary.unassignedGroupCount > 0) {
      expectedLimitations.push("topic_groups_unassigned");
    }
    if (
      summary.limitations.length !== expectedLimitations.length ||
      summary.limitations.some(
      ...
      ctx.addIssue({
        ...
        path: ["limitations"],
        message:
          "Generation limitation codes must exactly match non-zero gap counts",
      });
    }
  });
export type TopicModelGenerationSummary = z.infer<
  typeof TopicModelGenerationSummary
>;
...
const ConfirmedTopicModelRevision = z
  .object({
    ...
    confirmationMode: TopicModelConfirmationMode,
    contentHash: Sha256Hex,
    generationSummary: TopicModelGenerationSummary.nullable(),
  })
  .strict()
  .superRefine((model, ctx) => {
    const systemAuto = model.confirmationMode === "system_auto";
    if (systemAuto !== (model.confirmedBy === null)) {
      ctx.addIssue({
        code: "custom",
        path: ["confirmedBy"],
        message:
          "Only a system-auto confirmation has no human confirmation actor",
      });
    }
    if (systemAuto !== (model.generationSummary !== null)) {
      ctx.addIssue({
        code: "custom",
        path: ["generationSummary"],
        message:
          "Only a system-auto confirmation carries generated lineage",
      });
    }
  });
