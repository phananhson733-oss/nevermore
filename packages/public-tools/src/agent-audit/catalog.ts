...
const PAGE_TITLES: readonly CheckSeed[] = [
  ...
  ["8.7", "Static markup carries the page's own text", "静态标记是否承载页面自身正文", "The page is short on visible text AND its script payload dwarfs that text; only both together are a Tip. A long page with a proportionally large bundle is an ordinary application page, not an empty one. Read from the transferred markup — this run executes no script.", "页面可见正文偏少、且脚本体积远超这些正文——两个条件同时成立才为提示。正文长、包也按比例大的页面是正常的应用页，不是空页。依据传输到的标记判定——本次运行不执行脚本。"],
