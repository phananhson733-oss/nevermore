...
import { DRAFT_TOTAL_BUDGET_MS, DRAFT_V2_QUALITY_MAX, IMAGE_ALT_MAX_CHARS, IMAGE_PROMPT_MAX_CHARS, MODEL_TEXT_MAX_CHARS, OUTLINE_CAP, SECTION_ENDPOINT_BUDGET_MS, SECTION_MAX_ATTEMPTS, SECTION_MAX_SENTENCES, SENTENCE_MAX_CHARS } from "./constants.ts";
...
import { DRAFT_V2_MAX_BYTES, DRAFT_V2_SCHEMA, type DraftResultV2, type DraftV2Coverage, type DraftV2Quality, type DraftV2Rerun, type DraftV2Section, type DraftV2Settings, type DraftV2VerifyItem, type DraftV2ImagePrompts } from "./v2-draft-contract.ts";
import { draftV2Warnings } from "./v2-draft-prose.ts";
...
/** The same strip for the writing warnings, which a pre-checks draft does not carry. */
function splitQuality(input: Record<string, unknown>): { readonly rest: Record<string, unknown>; readonly quality: unknown } {
  if (!Object.hasOwn(input, "quality")) return { rest: input, quality: undefined };
  const { quality, ...rest } = input;
  return { rest, quality };
}
const qualityLocation = object({ section_id: id("O"), paragraph: count(SECTION_MAX_SENTENCES), sentence: nullable(count(SECTION_MAX_SENTENCES)) });
const qualityShape: Decoder<DraftV2Quality> = object({
  warnings: array(object({
    code: oneOf(["paragraph_too_long", "filler_phrase", "source_phrase_reused", "repeated_across_sections"] as const),
    at: qualityLocation, other: nullable(qualityLocation),
  }), { max: DRAFT_V2_QUALITY_MAX }),
});
...
function derive(sections: readonly DraftV2Section[], confirmed: ConfirmedBriefV2, settings: DraftV2Settings, coverage: DraftV2Coverage, rerun: DraftV2Rerun | null) {
  ...
  // Every one of these scopes was already built once to parse the section, so a
  // failure here is impossible rather than merely unlikely; skipping is still
  // the right answer, because a warning list is advisory and must never be the
  // thing that fails an otherwise valid draft.
  const prose = okSections.flatMap((section) => {
    const scope = buildDraftV2SectionScope(confirmed, section.id, settings);
    return scope.ok ? [{ id: section.id, body: section.body, evidence: scope.value }] : [];
  });
  const quality: DraftV2Quality = { warnings: draftV2Warnings(prose) };
  return { reads, verify, totals: measureResearchLength(allText.join(" "), confirmed.brief.context.input.language), mode, quality, llm: aggregateSectionLlm(sectionCalls(sections, rerun), 0.4) } as const;
}
...
export async function assembleDraftV2(input: AssembleDraftV2Input): Promise<Decoded<DraftResultV2>> {
  ...
  const derived = derive(sections.value, confirmed.value, settings.value, coverage.value.coverage, run.value.rerun);
...
async function parseBoundDraft(input: unknown, confirmed: ConfirmedBriefV2): Promise<Decoded<DraftResultV2>> {
  ...
  const settings = settingsShape(input.settings, "settings");
  if (!settings.ok) return settings;
  const { rest: withoutPlan, plan } = splitImagePrompts(input);
  const { rest, quality } = splitQuality(withoutPlan);
  ...
  const parsedQuality = quality === undefined ? undefined : qualityShape(quality, "quality");
  if (parsedQuality !== undefined && !parsedQuality.ok) return parsedQuality;
  const decoded: Decoded<DraftResultV2> = ok({
    ...bare.value,
    ...(imagePrompts === undefined ? {} : { image_prompts: imagePrompts.value }),
    ...(parsedQuality === undefined ? {} : { quality: parsedQuality.value }),
  });
  const value = decoded.value;
  ...
  const derived = derive(value.sections, confirmed, value.settings, value.coverage, value.run.rerun);
  // Absent means the draft predates these checks, and its fingerprint was taken
  // without them; present has to be exactly what this prose warrants.
  if (parsedQuality !== undefined && !same(parsedQuality.value, derived.quality)) return reference("quality");
